...
import { executeTestQuery } from './test-db';
import { getMode } from '@/lib/state/dashboardState';
import { getServerConfig, ServerConfig as ImportedServerConfig } from '@/lib/db/connections';
...
interface QueryResult {
  ...
  suggestion?: string;
  data?: any[]; // Add data property to support returning multiple rows
}

// Use the imported ServerConfig type to ensure compatibility
type ServerConfig = ImportedServerConfig;
  ...
  try {
    ...
    if (!mode || isTestMode) {
      try {
        ...
        const result = await executeTestQuery(transformedSql, extractedRowId);
    ...
    // Otherwise, use the production database
      ...
      try {
        ...
        const serverConfig = await getServerConfig(server as 'P21' | 'POR');
        ...
        if (server === 'P21') {
          ...
          result = await executeP21Query(sql, serverConfig);
        } else if (server === 'POR') {
          // Execute MS Access query
          result = await executePORQuery(sql, serverConfig);
        } else {
 ...
 */
async function executeP21Query(sql: string, serverConfig: ServerConfig): Promise<number> {
 ...
 */
async function executePORQuery(sql: string, serverConfig: ServerConfig): Promise<number> {
 ...
 */
function transformSqlForSqlite(sql: string, server: string): string {
