...
const porReaders = new Map<string, any>();
const odbcConnections = new Map<string, any>();
const sqlPools = new Map<string, sql.ConnectionPool>();

// Connection types
export type ServerType = 'P21' | 'POR' | 'SQLite';

// Connection information for different server types
...
export class ConnectionManager {
   ...
   */
  static async testConnection(serverType: string, config: ServerConfig): Promise<{ success: boolean; message: string }> {
    ...
    try {
      if (serverType === 'P21') {
        ...
        console.log('Testing P21 connection with config:', { 
          ...p21Config, 
          password: p21Config.password ? '***' : undefined 
        });
        
        return await this.testP21Connection(p21Config);
      } else if (serverType === 'POR') {
        if (!config.filePath) {
          return { success: false, message: 'File path is required for POR connections' };
        }
        
        return await this.testAccessConnection(config);
      } else {
   ...
   * @returns Result of the connection test
   */
  static async testAccessConnection(config: ServerConfig): Promise<{ success: boolean; message: string }> {
    try {
      if (!config.filePath) {
      ...
      const result = await this.executeAccessQuery(config, 'SELECT TOP 1 * FROM MSysObjects');
   ...
   */
  static async testP21Connection(config: {
    dsn?: string;
    username?: string;
    password?: string;
    database?: string;
   ...
   * @returns Result of the connection test
   */
  static async testSqlServerConnection(config: ServerConfig): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  static async getPORDirectReader(config: ServerConfig): Promise<any> {
    try {
      if (!config.filePath) {
   ...
   */
  static async executeAccessQuery(config: ServerConfig, query: string): Promise<any> {
    try {
      if (!config.filePath) {
      ...
      const reader = await this.getPORDirectReader(config);
      const result = await reader.executeQuery(query);
   ...
   */
  static async executePORQuery(query: string, porFilePath?: string): Promise<any[]> {
    try {
      ...
      const reader = await this.getConnection(config);
   ...
   */
  static async getConnection(config: ServerConfig): Promise<any> {
   ...
   */
  static getConnectionKey(config: ServerConfig): string {
    if (config.type === 'P21') {
      return 'P21';
  ...
  private static createPorConnectionPool(config: ServerConfig): Promise<sql.ConnectionPool> {
   ...
   * @returns Query results
   */
  static async executeP21Query(query: string): Promise<any> {
    ...
    try {
      ...
      try {
        ...
        // Don't replace empty results with a special indicator
        return result || [];
   ...
   * @returns Query result
   */
  async executeP21Query(config: ServerConfig, query: string): Promise<any> {
   ...
   * @returns Query result
   */
  async executeAccessQuery(config: ServerConfig, query: string): Promise<any> {
    return ConnectionManager.executeQuery(config, query);
  }
   ...
   */
  static async executeQueryWithFreshConnection(
      ...
      try {
        ...
        try {
          ...
          if (Array.isArray(queryResult) && queryResult.length > 0) {
            ...
            // Get the first row
            const firstRow = queryResult[0] as Record<string, any>;
   ...
   * @returns Query result
   */
  static async executeSqlServerQueryWithFreshConnection(config: ServerConfig, query: string): Promise<any> {
    try {
      ...
      const sqlConfig: any = {
        ...
        database: config.database,
        options: {
          encrypt: config.options?.encrypt ?? true, // Use encryption by default
   ...
   * @returns Query result
   */
  static async executeSqlServerQuery(config: ServerConfig, query: string): Promise<any> {
    try {
