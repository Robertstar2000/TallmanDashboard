...
export async function executeServerQuery(
  ...
  try {
    ...
    const { getServerConfig } = await import('./connections');
    ...
    const { getSqlServerClient } = await import('./sqlserver');
    const client = await getSqlServerClient(config);
...
export async function testServerConnection(
  ...
  try {
    ...
    console.log(`SERVER-OPS: Testing connection to ${server} server with config:`, 
      server === 'P21' 
 ...
 * This function should only be called from server components or API routes
 */
export async function closeServerConnection(
  server: 'P21' | 'POR'
): Promise<{ success: boolean; message: string }> {
  try {
    // This is a server-side only operation
