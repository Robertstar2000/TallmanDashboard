...
# This script directly updates the SQL expressions for P21 entries with the new format
...
$logPath = "C:\Users\BobM\CascadeProjects\TallmanDashboard_new\scripts\p21-sql-updates.log"

# Start logging
Start-Transcript -Path $logPath -Append
Write-Host "Starting P21 SQL expression updates at $(Get-Date)"
...
for ($i = 0; $i -lt $fileContent.Count; $i++) {
    ...
    elseif ($inEntry -and $currentEntry.IsP21 -and $line -match '"tableName":\s*"(.*?)"') {
    ...
    elseif ($inEntry -and $currentEntry.IsP21 -and $line -match '"productionSqlExpression":\s*"(.*?)"') {
        ...
        if ($sqlUpdates.ContainsKey($chartGroup)) {
            ...
            Write-Host "Updated SQL expression for ID $($currentEntry.Id) in chart group '$chartGroup'" -ForegroundColor Green
        }
    }
...
Write-Host "P21 SQL expression updates completed at $(Get-Date)"
Stop-Transcript
