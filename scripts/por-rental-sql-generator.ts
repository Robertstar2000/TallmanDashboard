...
async function main() {
  try {
    ...
    for (let i = 0; i < sqlExpressions.length; i++) {
      ...
      try {
        const result = await testSqlExpression(expression.sql);
        if (result.success && !result.error) {
          ...
          for (const table of availableTables) {
            if (fixed || table === 'Rentals') continue;
            ...
            const alternativeResult = await testSqlExpression(alternativeSql);
