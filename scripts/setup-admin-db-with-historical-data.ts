...
// Define the AdminVariable interface
...
async function setupAdminDbWithHistoricalData() {
  try {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      historicalDataRows.push({
        ...
        production_sql_expression: `SELECT ISNULL(SUM(order_amt), 0) as value FROM dbo.oe_hdr WITH (NOLOCK) WHERE FORMAT(order_date, 'yyyy-MM') = FORMAT(DATEADD(month, ${monthOffset}, GETDATE()), 'yyyy-MM')`,
        table_name: "oe_hdr",
        timeframe: `Month ${i}`
      });
    ...
    const additionalP21Metrics: AdminVariable[] = [
      {
        name: "Total Orders",
        value: "0",
        category: "P21",
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.oe_hdr WITH (NOLOCK)",
        table_name: "oe_hdr"
      },
      {
        name: "Open Orders",
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.oe_hdr WITH (NOLOCK) WHERE order_status = 'O'",
        table_name: "oe_hdr"
      },
      {
        ...
        production_sql_expression: "SELECT ISNULL(AVG(order_amt), 0) FROM dbo.oe_hdr WITH (NOLOCK)",
        table_name: "oe_hdr"
      },
      {
        name: "Total Invoices",
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.invoice_hdr WITH (NOLOCK)",
      ...
      },
      {
        ...
        production_sql_expression: "SELECT ISNULL(AVG(invoice_amt), 0) FROM dbo.invoice_hdr WITH (NOLOCK)",
        table_name: "invoice_hdr"
      },
      {
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.inv_mast WITH (NOLOCK)",
        table_name: "inv_mast"
      },
      {
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.inv_mast WITH (NOLOCK) WHERE status = 'A'",
        table_name: "inv_mast"
      },
      {
        ...
        production_sql_expression: "SELECT ISNULL(SUM(qty_on_hand * unit_cost), 0) FROM dbo.inv_loc WITH (NOLOCK)",
        table_name: "inv_loc"
      },
      {
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.customer WITH (NOLOCK)",
        table_name: "customer"
      },
      {
        ...
        production_sql_expression: "SELECT COUNT(*) FROM dbo.customer WITH (NOLOCK) WHERE status = 'A'",
        table_name: "customer"
      }
    ];
    ...
    const verifyResult = await executeWrite("SELECT COUNT(*) as count FROM admin_variables");
    ...
    const sampleP21Result = await executeWrite("SELECT * FROM admin_variables WHERE chart_name = 'Historical Data' AND variable_name = 'P21' LIMIT 1");
    const samplePORResult = await executeWrite("SELECT * FROM admin_variables WHERE chart_name = 'Historical Data' AND variable_name = 'POR' LIMIT 1");
    const sampleP21MetricResult = await executeWrite("SELECT * FROM admin_variables WHERE category = 'P21' AND chart_name != 'Historical Data' LIMIT 1");
    ...
    if (Array.isArray(sampleP21Result) && sampleP21Result.length > 0) {
      ...
      console.log(`Production SQL: ${sampleP21Result[0].production_sql_expression}`);
    ...
    if (Array.isArray(samplePORResult) && samplePORResult.length > 0) {
      ...
      console.log(`Production SQL: ${samplePORResult[0].production_sql_expression}`);
    ...
    if (Array.isArray(sampleP21MetricResult) && sampleP21MetricResult.length > 0) {
      ...
      console.log(`Production SQL: ${sampleP21MetricResult[0].production_sql_expression}`);
