// Script to fix the Open Invoices SQL expression with a working variation
const fs = require('fs');
...
fs.readFile(chartDataFilePath, 'utf8', (err, data) => {
  ...
  const backupFilePath = `${chartDataFilePath}.backup-final-fix-${new Date().toISOString().replace(/:/g, '-')}`;
