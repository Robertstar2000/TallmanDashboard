  ...
  try {
    ...
    for (const table of TABLES_TO_INSPECT) {
      ...
      if (rowCount > 0 && !sampleData && columns.length > 0) {
        ...
        const specificColumnsResponse = await testSql(specificColumnsSql);
      ...
      if (rowCount > 0) {
        ...
        if (dateColumns.length > 0) {
          ...
          for (const dateCol of dateColumns) {
            ...
            for (let month = 1; month <= 12; month++) {
              ...
              const monthResponse = await testSql(monthSql);
              ...
              if (monthResponse.success && monthResponse.result) {
                ...
                if (monthCount > 0) {
                  ...
                  const monthSampleResponse = await testSql(monthSampleSql);
        ...
        if (statusColumns.length > 0) {
          ...
          for (const statusCol of statusColumns) {
            ...
            const distinctResponse = await testSql(distinctSql);
            ...
            if (distinctResponse.success && distinctResponse.result) {
              ...
              for (const value of distinctValues) {
                ...
                const statusCountResponse = await testSql(statusCountSql);
