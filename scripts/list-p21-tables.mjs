// Script to list all tables in the P21Play database
import odbc from 'odbc';
import fs from 'fs';
import path from 'path';

async function listP21Tables() {
  let connection;
  try {
    ...
    // Get all tables in the database using INFORMATION_SCHEMA
    const tablesQuery = `
      SELECT 
    ...
    markdownContent += '## All Tables\n\n';
    markdownContent += '| Schema | Table Name |\n';
    markdownContent += '|--------|------------|\n';
    
    tablesResult.forEach(table => {
      markdownContent += `| ${table.SchemaName} | ${table.TableName} |\n`;
    });
    ...
    console.error('Error listing P21 tables:', error);
    return [];
  } finally {
    if (connection) {
      try {
