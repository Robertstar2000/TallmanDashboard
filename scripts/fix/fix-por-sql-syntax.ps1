...
foreach ($month in $monthMap.Keys) {
    ...
    $oldPattern1 = "sqlExpression`": `"SELECT Count\(\*\) as value FROM Transactions WHERE Month\(DateCreated\) = $monthNum AND Year\(DateCreated\) = Year\(Date\(\)\) AND DatePart\('yyyy', RentalDate\) = DatePart\('yyyy', Now\(\)\)"
    $oldPattern2 = "sqlExpression`": `"SELECT Count\(\*\) as value FROM Rentals WHERE Status = 'New' AND DatePart\('m', RentalDate\) = $monthNum AND DatePart\('yyyy', RentalDate\) = DatePart\('yyyy', Now\(\)\)"
    
    # Create the correct SQL
    $newSql = $newRentalsSql -f $monthNum
    $newPattern = "sqlExpression`": `"$newSql"
...
foreach ($month in $monthMap.Keys) {
    ...
    # Look for patterns that need to be fixed
    $oldPattern1 = "sqlExpression`": `"SELECT Count\(\*\) as value FROM Transactions WHERE TransactionStatus IN \('Open', 'Active', 'Out'\) AND Month\(DateCreated\) = $monthNum"
    $oldPattern2 = "sqlExpression`": `"SELECT Count\(\*\) as value FROM Rentals WHERE Status = 'Open' AND DatePart\('m', RentalDate\) = $monthNum"
...
foreach ($month in $monthMap.Keys) {
    ...
    # Look for patterns that need to be fixed
    $oldPattern1 = "sqlExpression`": `"SELECT Sum\(TotalAmount\) as value FROM Transactions WHERE TransactionStatus IN \('Open', 'Active', 'Out'\) AND Month\(DateCreated\) = $monthNum"
    $oldPattern2 = "sqlExpression`": `"SELECT Sum\(RentalValue\) as value FROM Rentals WHERE DatePart\('m', RentalDate\) = $monthNum"
