...
content = content.replace(/sqlExpression: `SELECT ISNULL\(SUM\(INVOICE_AMT\), 0\) as value FROM INVOICE_HDR WITH \(NOLOCK\) WHERE CONVERT\(date, INVOICE_DATE\) = CONVERT\(date, DATEADD\(day, -1, GETDATE\(\)\)\)`, 0\) as value FROM invoice_hdr`/g,
