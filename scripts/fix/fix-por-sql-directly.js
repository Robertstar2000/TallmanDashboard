...
function fixPorSqlExpressions() {
  ...
  for (const [variableName, template] of Object.entries(porSqlTemplates)) {
    for (const [monthName, monthNumber] of Object.entries(monthMap)) {
      ...
      const pattern = new RegExp(
        `(name:\\s*["']Por Overview \\d+["'][^}]*variableName:\\s*["']${variableName}["'][^}]*serverName:\\s*['"]POR['"][^}]*${monthName}[^}]*sqlExpression:\\s*["'])([^"']*)(['"])`,
        'i'
      );
  ...
  for (const [agingBucket, sql] of Object.entries(arAgingSqlTemplates)) {
    ...
    const pattern = new RegExp(
      `(name:\\s*["']AR Aging[^}]*variableName:\\s*["']${agingBucket}["'][^}]*serverName:\\s*['"]POR['"][^}]*sqlExpression:\\s*["'])([^"']*)(['"])`,
      'i'
    );
  ...
  content = content.replace(
    /(serverName:\s*['"]POR['"][^}]*sqlExpression:\s*["'])([^"']*dbo\.[^"']*WITH\s*\(\s*NOLOCK\s*\)[^"']*)(['"])/g,
...
function validatePorSql(content) {
  ...
  const porSqlServerSyntaxPattern = /serverName:\s*['"]POR['"][^}]*sqlExpression:\s*["']([^"']*(?:dbo\.|WITH\s*\(\s*NOLOCK\s*\)|GETDATE\(\)|DATEADD|DATEDIFF)[^"']*)["']/g;
