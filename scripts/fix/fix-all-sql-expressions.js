...
function fixSqlExpression(line) {
  ...
  if (line.includes('sqlExpression:') || line.includes('sqlExpression:')) {
    // Extract the SQL expression part
    const parts = line.split(/sqlExpression:|sqlExpression:/);
    const prefix = parts[0] + (line.includes('sqlExpression:') ? 'sqlExpression:' : 'sqlExpression:');
    let sqlPart = parts[1];
...
finalContent = finalContent.replace(/sqlExpression: ('[^']*')(\s*\n\s*tableName)/g, 'sqlExpression: $1,$2');
...
finalContent = finalContent.replace(/sqlExpression: 'SELECT COALESCE\(SUM\(QTY_ON_HAND\), 0\) as value[^']*?(?=\s*,|\s*\n)/g, 
                     "sqlExpression: 'SELECT COALESCE(SUM(QTY_ON_HAND), 0) as value FROM INV_MAST im JOIN PRODUCT_LINE pl ON im.PROD_LINE = pl.PROD_LINE WHERE pl.DESCRIPTION LIKE \"%Plumbing%\"'");

finalContent = finalContent.replace(/sqlExpression: 'SELECT COALESCE\(SUM\(OPEN_AMT\)\)', 0\) as value/g, 
                     "sqlExpression: 'SELECT COALESCE(SUM(OPEN_AMT), 0) as value");
