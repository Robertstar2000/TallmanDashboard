...
import { singleSourceData as initialSpreadsheetData } from '../../lib/db/single-source-data';
...
const COLUMN_NAME_MAP: Record<string, Record<string, string>> = {
 ...
 */
export function fixPORSqlExpression(sql: string, availableTables: string[] = []): string {
  if (!sql) return sql;
 ...
 */
export function getAllPORSqlExpressions(): Array<{id: string, dataPoint: string, sql: string}> {
  return initialSpreadsheetData
    .filter(item => item.serverName === 'POR' && typeof item.productionSqlExpression === 'string')
    .map(item => ({
      id: item.rowId,
      dataPoint: item.DataPoint || `POR Expression ${item.rowId}`,
      sql: item.productionSqlExpression as string
    }));
}
 ...
 */
export function fixAllPORSqlExpressions(availableTables: string[] = []): Array<{
  id: string, 
  dataPoint: string, 
  originalSql: string, 
  fixedSql: string
