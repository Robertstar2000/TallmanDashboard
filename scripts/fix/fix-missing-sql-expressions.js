 ...
 * This script identifies all rows with "number" as sql_expression
...
db.all("SELECT id, chart_group, variable_name, server_name FROM chart_data WHERE sql_expression = 'number'", [], (err, rows) => {
  if (err) {
  ...
  db.run('BEGIN TRANSACTION', (err) => {
    ...
    rows.forEach((row) => {
      ...
      db.run(
        "UPDATE chart_data SET sql_expression = ? WHERE id = ?",
...
fs.readFile(singleSourceDataPath, 'utf8', (err, data) => {
  ...
  const updatedRowStrings = rowStrings.map((rowString) => {
    ...
    if (rowString.includes('"sqlExpression": "number"')) {
      ...
      if (chartGroupMatch && variableNameMatch) {
        ...
        return rowString.replace(
          ...
          `"sqlExpression": ${JSON.stringify(sqlExpression)}`
        );
      }
    }
