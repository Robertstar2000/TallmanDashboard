...
function setupPorHistoricalData() {
  try {
    console.log('Setting up POR Historical Data rows...');
    
    // Create a temporary script to set up the POR Historical Data rows
    const scriptContent = `
    ...
    fs.writeFileSync('temp-setup-historical-data.ts', scriptContent);
    
    // Execute the setup script
    console.log('Executing setup script...');
    const output = execSync('npx ts-node temp-setup-historical-data.ts', { 
      cwd: process.cwd(),
      encoding: 'utf8'
    });
    ...
    fs.unlinkSync('temp-setup-historical-data.ts');
