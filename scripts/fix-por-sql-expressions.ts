...
import { porSchema, getPORTableNames, getPORTableColumnNames } from '../lib/db/por-schema';
...
async function main() {
  try {
    ...
    async function processNextExpression() {
      ...
      const testResult = await testSqlExpression(expression.productionSqlExpression);
      ...
      async function tryNextTable() {
        ...
        for (let i = 0; i < Math.min(CONFIG.MAX_COLUMN_TRIES, rankedColumns.length); i++) {
          ...
          const result = await testSqlExpression(generatedSql);
            ...
            if (!bestResult || (result.value !== null && result.value !== undefined)) {
              ...
              rl.question('Accept this SQL expression? (y/n): ', async (answer) => {
                if (answer.toLowerCase() === 'y') {
                  ...
                  const fixedExpression = { ...expression, productionSqlExpression: bestSql };
