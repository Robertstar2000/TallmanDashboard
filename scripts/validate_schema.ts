...
interface SchemaDefinition {
  [tableName: string]: string[]; // Table name maps to list of column names
}
...
const p21Schema: SchemaDefinition = {};
...
const porSchema: SchemaDefinition = {
...
async function testP21(elementType: 'Table' | 'Column', name: string, tableName?: string): Promise<TestResult> {
    let query = '';
    const baseResult = { name, db: 'P21' as const, type: elementType };
...
async function testPOR(elementType: 'Table' | 'Column', name: string, porFilePath: string, porPassword?: string, tableName?: string): Promise<TestResult> {
    let query = '';
    const baseResult = { name, db: 'POR' as const, type: elementType };
