...
rows.forEach((row) => {
    if (row.chartGroup === 'POR Overview') {
        ...
        if (row.variableName === 'New Rentals') {
            ...
            row.sqlExpression = `SELECT Count(*) as value FROM Transactions WHERE Month(DateCreated) = ${monthNumber} AND Year(DateCreated) = Year(Date())`;
        }
        else if (row.variableName === 'Open Rentals') {
            // Update for Open Rentals
            row.tableName = 'Transactions';
            row.sqlExpression = `SELECT Count(*) as value FROM Transactions WHERE TransactionStatus IN ('Open', 'Active', 'Out')`;
            // Add month filter if needed
            if (monthNumber) {
                row.sqlExpression = `SELECT Count(*) as value FROM Transactions WHERE TransactionStatus IN ('Open', 'Active', 'Out') AND Month(DateCreated) = ${monthNumber}`;
            }
        }
        else if (row.variableName === 'Rental Value') {
            // Update for Rental Value
            row.tableName = 'Transactions';
            row.sqlExpression = `SELECT Sum(TotalAmount) as value FROM Transactions WHERE TransactionStatus IN ('Open', 'Active', 'Out')`;
            // Add month filter if needed
            if (monthNumber) {
                row.sqlExpression = `SELECT Sum(TotalAmount) as value FROM Transactions WHERE TransactionStatus IN ('Open', 'Active', 'Out') AND Month(DateCreated) = ${monthNumber}`;
            }
        }
        // Update lastUpdated
