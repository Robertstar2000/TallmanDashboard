...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const rentalTables = tableNames.filter(table => table.toLowerCase().includes('rent') ||
                            ...
                            if (rows.length > 0) {
                                for (const statusCol of statusColumns) {
                                    ...
                                    for (let i = 0; i < Math.min(rows.length, 100); i++) {
                                        ...
                                        if (value)
                                            statusValues.add(String(value).toLowerCase());
                                    }
                                    // Check for open/active status values
                                    const openStatuses = Array.from(statusValues).filter(val => typeof val === 'string' && (val.includes('open') ||
                                        val.includes('active') ||
