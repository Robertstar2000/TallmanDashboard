...
const methodDefaulted = jest.fn(function (one, two = _default) {
    return { one, two };
});
...
describe('moize', () => {
    ...
    describe('main', () => {
        ...
        it('should handle a curried call of options creation', () => {
            ...
            expect(moized.options).toEqual(expect.objectContaining({
                isSerialized: true,
                maxAge: 1000,
                maxSize: 5,
            }));
        ...
        it('should handle moizing an already-moized function with additional options', () => {
            ...
            expect(moized.options).toEqual(Object.assign(Object.assign({}, memoized.options), { maxSize: 5 }));
        });
    });
    describe('cache manipulation', () => {
        ...
        it('should notify of cache change on removal and clear the expiration', () => {
            ...
            expect(result).toBe(true);
            expect(expiringMemoized.options.onCacheChange).toHaveBeenCalledWith(expiringMemoized.cache, expiringMemoized.options, expiringMemoized);
        ...
        it('should notify of the cache change on clear', () => {
            ...
            expect(result).toBe(true);
            expect(changeMemoized.options.onCacheChange).toHaveBeenCalledWith(changeMemoized.cache, changeMemoized.options, changeMemoized);
        });
    ...
    describe('properties', () => {
        it('should have the micro-memoize options', () => {
            ...
            const _a = memoized._microMemoizeOptions, { isEqual } = _a, options = __rest(_a, ["isEqual"]);
            const _b = mmResult.options, { isEqual: _isEqualIgnored } = _b, resultOptions = __rest(_b, ["isEqual"]);
            expect(options).toEqual(resultOptions);
            expect(isEqual).toBe(sameValueZeroEqual);
        });
        it('should have cache and cacheSnapshot', () => {
            memoized(foo, bar);
            expect(memoized.cache).toEqual(expect.objectContaining({
                keys: [[foo, bar]],
                values: [{ one: foo, two: bar }],
            }));
            expect(memoized.cache.size).toBe(1);
            expect(memoized.cacheSnapshot).toEqual(expect.objectContaining({
                keys: [[foo, bar]],
                values: [{ one: foo, two: bar }],
            }));
