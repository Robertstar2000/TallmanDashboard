...
import { useSelectRange } from './SelectRangeContext';
const today = new Date(2021, 11, 8);
freezeBeforeAll(today);
function renderHook(props) {
    return renderDayPickerHook(useSelectRange, props);
}
...
describe('when no days are selected', () => {
    ...
    describe('when "onDayClick" is called', () => {
        ...
        beforeAll(() => {
            ...
            (_b = (_a = result.current).onDayClick) === null || _b === void 0 ? void 0 : _b.call(_a, day, activeModifiers, stubEvent);
        });
        afterAll(() => {
            jest.resetAllMocks();
        });
        ...
        test('should call "onSelect" with the clicked day as the "from" prop', () => {
            expect(initialProps.onSelect).toHaveBeenCalledWith({ from: day, to: undefined }, day, activeModifiers, stubEvent);
        });
    });
});
...
describe('when a complete range of days is selected', () => {
    ...
    describe('when "onDayClick" is called with the day before the from day', () => {
        ...
        test('should call "onSelect" with the day selected', () => {
            expect(dayPickerProps.onSelect).toHaveBeenCalledWith({ from: day, to }, day, activeModifiers, stubEvent);
        });
    });
});
...
describe('when the max number of the selected days is reached', () => {
    ...
    const to = addDays(today, 6);
    const selected = { from, to };
    const dayPickerProps = Object.assign(Object.assign({}, initialProps), { selected, max: 7 });
...
describe('when the minimum number of days are selected', () => {
    const selected = { from, to };
    const dayPickerProps = Object.assign(Object.assign({}, initialProps), { selected, min: Math.abs(differenceInCalendarDays(to, from)) });
    ...
    describe('when "onDayClick" is called with the "from" day', () => {
        ...
        test('should call "onSelect" with an undefined range', () => {
            expect(dayPickerProps.onSelect).toHaveBeenCalledWith(undefined, day, activeModifiers, stubEvent);
        });
    });
    describe('when "onDayClick" is called with the "to" day', () => {
        ...
        test('should call "onSelect" without the "to" in the range', () => {
            expect(dayPickerProps.onSelect).toHaveBeenCalledWith({ from: day, to: undefined }, day, activeModifiers, stubEvent);
        });
    });
});
