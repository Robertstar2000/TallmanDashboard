...
describe("loadConfig", () => {
    ...
    it("should throw an error including the file path when encountering invalid JSON5", () => {
        expect(() => loadTsconfig("/root/dir1/tsconfig.json", (path) => path === "/root/dir1/tsconfig.json", (_) => `{
            "compilerOptions": {
    ...
    it("should use baseUrl relative to location of extended tsconfig", () => {
        ...
        const res = loadTsconfig(join("/root", "dir1", "dir2", "third-config.json"), (path) => path === firstConfigPath ||
    ...
    it("should load a config with array extends and overwrite all options", () => {
        ...
        const res = loadTsconfig(join("/root", "tsconfig.json"), (path) => [
            baseConfig1Path,
            baseConfig2Path,
            baseConfig3Path,
            actualConfigPath,
    ...
    it("should load a config with array extends without .json extension", () => {
        ...
        const actualConfigPath = join("/root", "tsconfig.json");
        const res = loadTsconfig(join("/root", "tsconfig.json"), (path) => [baseConfigPath, actualConfigPath].indexOf(path) >= 0, (path) => {
            if (path === baseConfigPath) {
                return JSON.stringify(baseConfig);
            }
            if (path === actualConfigPath) {
