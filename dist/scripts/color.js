...
/* constants */
import { ANGLE, CS_HUE_CAPT, CS_MIX, CS_RGB, CS_XYZ, FN_COLOR, FN_MIX, NONE, NUM, PCT, SYN_COLOR_TYPE, SYN_FN_COLOR, SYN_HSL, SYN_HSL_LV3, SYN_LCH, SYN_MIX, SYN_MIX_CAPT, SYN_MIX_PART, SYN_MOD, SYN_RGB_LV3, VAL_COMP, VAL_MIX, VAL_SPEC } from './constant';
 ...
 */
export const cacheInvalidColorValue = (cacheKey, format, nullable = false) => {
    if (format === VAL_SPEC) {
        const res = '';
 ...
 */
export const resolveInvalidColorValue = (format, nullable = false) => {
    switch (format) {
        case 'hsl':
        case 'hwb':
        case VAL_MIX: {
 ...
 */
export const transformRgbToLinearRgb = (rgb, skip = false) => {
    let rr, gg, bb;
    if (skip) {
        [rr, gg, bb] = rgb;
    }
 ...
 */
export const transformRgbToXyz = (rgb, skip = false) => {
    if (!skip) {
 ...
 * @returns TriColorChannels - [x, y, z]
 */
export const transformRgbToXyzD50 = (rgb) => {
 ...
 */
export const transformLinearRgbToRgb = (rgb, round = false) => {
 ...
 * @returns TriColorChannels - [r, g, b] r|g|b: 0..255
 */
export const transformXyzToRgb = (xyz, skip = false) => {
    if (!skip) {
 ...
 * @returns TriColorChannels - [x, y, z]
 */
export const transformXyzToXyzD50 = (xyz) => {
    xyz = validateColorComponents(xyz, {
        maxLength: TRIA,
        validateRange: false
    });
 ...
 */
export const transformXyzToHsl = (xyz, skip = false) => {
 ...
 */
export const transformXyzToHwb = (xyz, skip = false) => {
 ...
 */
export const transformXyzToOklab = (xyz, skip = false) => {
    if (!skip) {
        xyz = validateColorComponents(xyz, {
            maxLength: TRIA,
            validateRange: false
 ...
 */
export const transformXyzToOklch = (xyz, skip = false) => {
 ...
 * @returns TriColorChannels - [r, g, b] r|g|b: 0..255
 */
export const transformXyzD50ToRgb = (xyz, skip = false) => {
    if (!skip) {
        xyz = validateColorComponents(xyz, {
            maxLength: TRIA,
            validateRange: false
 ...
 * @returns TriColorChannels - [l, a, b]
 */
export const transformXyzD50ToLab = (xyz, skip = false) => {
    if (!skip) {
        xyz = validateColorComponents(xyz, {
            maxLength: TRIA,
            validateRange: false
 ...
 * @returns TriColorChannels - [l, c, h]
 */
export const transformXyzD50ToLch = (xyz, skip = false) => {
...
export const parseHsl = (value, opt = {}) => {
    ...
    const gk = (8 + h) % DOZ;
    const bk = (4 + h) % DOZ;
    const r = l - sa * Math.max(-1, Math.min(rk - TRIA, Math.pow(TRIA, POW_SQR) - rk, 1));
    const g = l - sa * Math.max(-1, Math.min(gk - TRIA, Math.pow(TRIA, POW_SQR) - gk, 1));
    const b = l - sa * Math.max(-1, Math.min(bk - TRIA, Math.pow(TRIA, POW_SQR) - bk, 1));
    return [
        'rgb',
...
export const resolveColorMix = (value, opt = {}) => {
    ...
    if (/^srgb(?:-linear)?$/.test(colorSpace)) {
        ...
        const [[rA, gA, bA, alphaA], [rB, gB, bB, alphaB]] = normalizeColorComponents([rrA, ggA, bbA, aaA], [rrB, ggB, bbB, aaB], true);
        ...
        const alphaNone = aaA === NONE && aaB === NONE;
        const [[xA, yA, zA, alphaA], [xB, yB, zB, alphaB]] = normalizeColorComponents([xxA, yyA, zzA, aaA], [xxB, yyB, zzB, aaB], true);
        const factorA = alphaA * pA;
        const factorB = alphaB * pB;
        alpha = factorA + factorB;
        let x, y, z;
        ...
        const alphaNone = aaA === NONE && aaB === NONE;
        let [[hA, sA, lA, alphaA], [hB, sB, lB, alphaB]] = normalizeColorComponents([hhA, ssA, llA, aaA], [hhB, ssB, llB, aaB], true);
        if (hueArc) {
        ...
        const alphaNone = aaA === NONE && aaB === NONE;
        let [[lA, cA, hA, alphaA], [lB, cB, hB, alphaB]] = normalizeColorComponents([llA, ccA, hhA, aaA], [llB, ccB, hhB, aaB], true);
        if (hueArc) {
            [hA, hB] = interpolateHue(hA, hB, hueArc);
        }
        const factorA = alphaA * pA;
        ...
        const [[lA, aA, bA, alphaA], [lB, aB, bB, alphaB]] = normalizeColorComponents([llA, aaA, bbA, alA], [llB, aaB, bbB, alB], true);
        const factorA = alphaA * pA;
        const factorB = alphaB * pB;
        alpha = factorA + factorB;
        let l, aO, bO;
