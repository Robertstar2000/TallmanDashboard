...
it('does not interfere with default constructors', () => {
    const ProxyClass = createProxy(class {
        constructor(name) {
            this.name = name;
        }
    }, {});
...
it('spies on the constructor', () => {
    ...
    expect(constructorCall).toHaveBeenCalledWith(['John', 32], expect.any(Function));
});
...
it('spies on property setters', () => {
    ...
    expect(setProperty).toHaveBeenCalledWith(['foo', 'next'], expect.any(Function));
});
it('spies on method calls', () => {
    ...
    expect(methodCall).toHaveBeenCalledWith(['greet', ['Clair']], expect.any(Function));
});
