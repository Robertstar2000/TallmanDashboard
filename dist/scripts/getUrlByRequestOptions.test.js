...
it('returns a URL based on the basic RequestOptions', () => {
    expect(getUrlByRequestOptions({
        ...
        host: '127.0.0.1',
        path: '/resource',
    }).href).toBe('https://127.0.0.1/resource');
});
...
it('inherits protocol and port from https.Agent, if set', () => {
    ...
    }).href).toBe('https://127.0.0.1:3080/');
});
...
it('resolves protocol to "https" given no explicit protocol, but certificate', () => {
    ...
    }).href).toBe('https://127.0.0.1/secure');
});
...
it('supports "hostname" and "port"', () => {
    ...
    expect(getUrlByRequestOptions(options).href).toBe('https://127.0.0.1:1234/resource');
});
...
it('parses "host" and "port" in IPv6', () => {
    expect(getUrlByRequestOptions({
        ...
        port: 3001,
        path: '/resource',
    }).href).toBe('http://[::1]:3001/resource');
});
