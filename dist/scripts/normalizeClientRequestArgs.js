...
import { Agent as HttpAgent, globalAgent as httpGlobalAgent, } from 'node:http';
 ...
 */
URL, parse as parseUrl, } from 'node:url';
...
export function normalizeClientRequestArgs(defaultProtocol, args) {
        ...
        if (legacyUrl.hostname === null) {
            ...
            return isObject(args[1])
                ? normalizeClientRequestArgs(defaultProtocol, [
                    Object.assign({ path: legacyUrl.path }, args[1]),
                    args[2],
                ])
                : normalizeClientRequestArgs(defaultProtocol, [
                    { path: legacyUrl.path },
    ...
    if (typeof options.agent === 'undefined') {
        const agent = options.protocol === 'https:'
            ? new HttpsAgent(Object.assign({}, ('rejectUnauthorized' in options && {
