...
function generateMonthlyData(startId, prefix, baseValue, variance = 0.2) {
...
const accountsPayableData = Array.from({ length: 12 }, (_, i) => {
    const date = new Date(2025, 0, 1);
    date.setMonth(date.getMonth() - i);
    const monthStr = date.toISOString().slice(0, 7);
...
const dailyShipmentsData = Array.from({ length: 7 }, (_, i) => {
...
function initializeRawData(initialData) {
    // Clear existing data
