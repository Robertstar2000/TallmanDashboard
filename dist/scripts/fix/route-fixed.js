 ...
 */
function executeAccessQuery(sql, filePath) {
...
`;
            ...
            const { stdout, stderr } = yield execPromise(`powershell -ExecutionPolicy Bypass -File "${scriptPath}" -filePath "${filePath}" -sql "${sql.replace(/"/g, '\\"')}"`);
            if (stderr) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (server !== 'P21' && server !== 'POR') {
                ...
                return NextResponse.json({ error: 'Invalid server parameter. Must be P21 or POR' }, { status: 400 });
            }
            // Basic SQL validation
            if (!validateSql(sql)) {
                console.error(`API: Invalid SQL expression: ${sql}`);
                return NextResponse.json({ error: 'Invalid SQL expression. Only SELECT statements are allowed.' }, { status: 400 });
            }
            ...
            if (!isProdMode) {
                ...
                try {
                    const value = yield executeTestQuery(sql, server);
                ...
                try {
                    ...
                    if (!config) {
                        ...
                        return NextResponse.json({ error: `No configuration found for ${server} server` }, { status: 500 });
                    }
                    ...
                    if (server === 'POR' && config.filePath) {
                        ...
                        if (!fs.existsSync(config.filePath)) {
                            ...
                            return NextResponse.json({ error: `MS Access file not found at path: ${config.filePath}` }, { status: 404 });
                        }
                        try {
                            ...
                            const result = yield executeAccessQuery(accessSql, config.filePath);
                            ...
                            return NextResponse.json({ error: error instanceof Error ? error.message : 'Access database query failed' }, { status: 500 });
                        }
                    }
                    else {
                        ...
                        if (connectionId && connectionPool[connectionId]) {
                            ...
                            if (!status.isConnected) {
                                ...
                                return NextResponse.json({ error: `Failed to connect to ${server} server: ${status.error}` }, { status: 500 });
                            }
                    ...
                    return NextResponse.json({ error: error instanceof Error ? error.message : 'Database query failed' }, { status: 500 });
                }
            }
        }
        catch (error) {
            console.error('API: Unexpected error:', error);
            return NextResponse.json({ error: error instanceof Error ? error.message : 'Unexpected error occurred' }, { status: 500 });
        }
    });
}
