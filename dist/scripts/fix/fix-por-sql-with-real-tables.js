    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            for (let i = 0; i < porExpressions.length; i++) {
                ...
                const originalResult = yield testSqlExpression(expression.sqlExpression);
                ...
                if (!fixed) {
                    ...
                    if (dataPoint.toLowerCase().includes('rental') || dataPoint.toLowerCase().includes('por')) {
                        ...
                        if (dataPoint.toLowerCase().includes('open rental')) {
                            ...
                            const result = yield testSqlExpression(openRentalsPattern);
                            if (result.success && !result.error) {
                        ...
                        // Pattern 2: New rentals by month - count all with status of Open for a specific month
                            ...
                            const result = yield testSqlExpression(newRentalsPattern);
                            if (result.success && !result.error) {
                        ...
                        // Pattern 3: Rental value - sum Amount for all with status of Open for the current year
                            ...
                            const result = yield testSqlExpression(rentalValuePattern);
                            if (result.success && !result.error) {
                        ...
                        // Pattern 4: Historical OPOR value - sum Amount for all with status of Open for the current year
                            ...
                            const result = yield testSqlExpression(historicalValuePattern);
                            if (result.success && !result.error) {
                        ...
                        // Fallback pattern for any rental-related data point
                            ...
                            const result = yield testSqlExpression(fallbackPattern);
                            if (result.success && !result.error) {
                    ...
                    if (!fixed) {
                        ...
                        for (const table of availableTables) {
                            ...
                            for (const pattern of patterns) {
                                ...
                                const result = yield testSqlExpression(pattern);
                                if (result.success && !result.error) {
                ...
                if (fixed) {
                    ...
                    console.log(`Result: ${fixedResult === null || fixedResult === void 0 ? void 0 : fixedResult.value}`);
                    fixedCount++;
                    results.push({
                        id: expression.id,
                        dataPoint: expression.DataPoint || expression.name,
                        ...
                        working: true,
                        newSql: fixedSql,
                        value: fixedResult === null || fixedResult === void 0 ? void 0 : fixedResult.value
                    });
                }
                else {
 ...
 */
function findSimilarTables(extractedTable, availableTables) {
    if (!extractedTable)
 ...
 */
function fixSqlExpression(originalSql, tableName, extractedColumns) {
    if (!originalSql)
        return '';
