...
export function HelpDialog({ open, onOpenChange }) {
    ...
    const handleFileSelect = (file) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const response = yield fetch(`/api/docs?path=${encodeURIComponent(file.path)}`);
            if (response.ok) {
    ...
    return (<Dialog open={open} onOpenChange={handleDialogOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {showFileContent ? `Viewing: ${selectedFile === null || selectedFile === void 0 ? void 0 : selectedFile.name}` : 'Select a documentation file to view'}
          </DialogDescription>
        </DialogHeader>

        {!showFileContent ? (<div className="grid grid-cols-3 gap-4 mt-4">
            {docFiles.map((file) => (<Card key={file.path} className="p-4 hover:bg-gray-100 cursor-pointer transition-colors" onClick={() => handleFileSelect(file)}>
            ...
            <Button variant="outline" onClick={handleBackToFileList} className="mb-4">
              ← Back to Documentation Menu
            </Button>
            
            <div className="border rounded-md p-4 bg-gray-50">
              {(selectedFile === null || selectedFile === void 0 ? void 0 : selectedFile.type) === 'image' ? (<div className="flex justify-center">
                  {selectedFile.path.endsWith('.svg') ? (<div className="w-full h-[600px] overflow-auto bg-white p-4 rounded-md">
                      <iframe src={selectedFile.path} width="100%" height="100%" className="border-0" title="Data Flow Diagram"/>
                    </div>) : (<Image src={selectedFile.path} alt="Documentation Image" width={700} height={500} className="object-contain"/>)}
                </div>) : (<pre className="whitespace-pre-wrap font-mono text-sm overflow-auto max-h-[60vh]">
                  {fileContent}
                </pre>)}
            </div>
          </div>)}
