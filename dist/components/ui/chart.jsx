...
import { Tooltip as RechartsPrimitiveTooltip, Legend, ResponsiveContainer } from 'recharts';
...
const ChartTooltipContent = React.forwardRef((_a, ref) => {
    var { active, payload, className, indicator = 'dot', hideLabel = false, hideIndicator = false, label, labelKey, nameKey, color } = _a, props = __rest(_a, ["active", "payload", "className", "indicator", "hideLabel", "hideIndicator", "label", "labelKey", "nameKey", "color"]);
    const { config } = useChart();
    const tooltipLabel = React.useMemo(() => {
        var _a;
        if (!(payload === null || payload === void 0 ? void 0 : payload.length) || hideLabel) {
            return null;
        }
        ...
        if (props.labelFormatter) {
            return (<div className={cn('font-medium', props.labelClassName)}>
    ...
    return (<div ref={ref} className={cn('grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl', className)}>
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            return (<div key={typeof item.dataKey === 'string' ? item.dataKey : `tooltip-item-${index}`} className={cn('flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground', indicator === 'dot' && 'items-center')}>
                {props.formatter && (item === null || item === void 0 ? void 0 : item.value) !== undefined && item.name ? (props.formatter(item.value, item.name, item, index, item.payload)) : (<>
...
const ChartLegendContent = React.forwardRef(({ className, hideIcon = false, payload, verticalAlign = 'bottom', nameKey, color }, ref) => {
    const { config } = useChart();
    if (!(payload === null || payload === void 0 ? void 0 : payload.length)) {
        return null;
    }
    return (<div ref={ref} className={cn('flex flex-wrap gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)} data-vertical-align={verticalAlign}>
        {payload.map((item, index) => {
            const key = `${nameKey || item.dataKey || 'value'}`;
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            return (<div key={typeof item.dataKey === 'string' ? item.dataKey : `legend-item-${index}`} className={cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground')}>
