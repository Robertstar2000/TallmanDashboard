...
export function ChartsSection({ data }) {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <Card className="p-4">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data.accounts}>
            ...
            <XAxis dataKey="date" interval={0} angle={-45} textAnchor="end" height={60} tick={{ fontSize: 12 }} padding={{ left: 10, right: 10 }} tickFormatter={(value, index) => value} reversed={false}/>
            <YAxis yAxisId="left" domain={[0, 'dataMax']} tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`}/>
            <YAxis yAxisId="right" orientation="right" domain={[0, 'dataMax']} tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`}/>
            <Tooltip formatter={(value) => `$${Number(value).toLocaleString()}`}/>
            <Legend />
            <Line yAxisId="left" type="monotone" dataKey="payable" name="Payable" stroke="#8884d8" activeDot={{ r: 8 }}/>
            <Line yAxisId="left" type="monotone" dataKey="receivable" name="Receivable" stroke="#82ca9d" activeDot={{ r: 8 }}/>
            <Line yAxisId="right" type="monotone" dataKey="overdue" name="Overdue" stroke="#ffc658" activeDot={{ r: 8 }}/>
            {/* Add the Amount Due line if available */}
            {data.accounts.some(item => item.amountDue !== undefined) && (<Line yAxisId="left" type="monotone" dataKey="amountDue" name="Amount Due" stroke="#ff7300" activeDot={{ r: 8 }}/>)}
            {/* Add the Current line if available */}
            {data.accounts.some(item => item.current !== undefined) && (<Line yAxisId="left" type="monotone" dataKey="current" name="Current" stroke="#0088FE" activeDot={{ r: 8 }}/>)}
          </LineChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card className="p-4">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data.historicalData}>
            <CartesianGrid strokeDasharray="3 3"/>
            <XAxis dataKey="date" interval={0} angle={-45} textAnchor="end" height={60} tickFormatter={(value, index) => value} reversed={false}/>
      ...
      <Card className="p-4">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data.inventory}>
            <CartesianGrid strokeDasharray="3 3"/>
            <XAxis dataKey="department" interval={0} angle={-45} textAnchor="end" height={60}/>
            <YAxis />
            <Tooltip />
            <Legend />
      ...
      <Card className="p-4">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie data={data.siteDistribution} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" nameKey="name" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
              {data.siteDistribution.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
            </Pie>
            <Tooltip formatter={(value) => Number(value).toLocaleString()}/>
            <Legend />
          </PieChart>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data.arAging}>
            ...
            <Bar dataKey="amount" fill="#8884d8">
              {data.arAging.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </Card>
