...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger, DialogFooter, DialogClose, } from "@/components/ui/custom-dialog";
...
export function ServerConnectionDialog({ serverType, trigger, onConnectionChange }) {
    ...
    const [formData, setFormData] = React.useState(serverType === 'P21' ? defaultP21Config : defaultPORConfig);
    ...
    const handleSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (isConnected) {
            try {
                const success = yield connectToServer(serverType, null);
        ...
        try {
            const success = yield connectToServer(serverType, databaseConfig);
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => {
            const newData = Object.assign(Object.assign({}, prev), { [name]: value });
    ...
    return (<Dialog open={showDialog} onOpenChange={setShowDialog}>
      <DialogTrigger asChild onClick={() => setShowDialog(true)}>
        {isLoading ? (<Button variant="outline" disabled>
            Loading...
          </Button>) : (<Button variant={isConnected ? "default" : "outline"} className={isConnected ? "bg-green-600 hover:bg-green-700" : ""}>
        ...
        <div className="p-4 pb-0">
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            <div className="grid grid-cols-3 items-center gap-2">
              ...
              <Input id="serverName" name="serverName" value={formData.serverName} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="Server Name"/>
            </div>
            <div className="grid grid-cols-3 items-center gap-2">
              <Label htmlFor="ipAddress" className="text-xs">IP Address</Label>
              <Input id="ipAddress" name="ipAddress" value={formData.ipAddress} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="192.168.1.100"/>
            </div>
            <div className="grid grid-cols-3 items-center gap-2">
              <Label htmlFor="port" className="text-xs">Port</Label>
              <Input id="port" name="port" value={formData.port} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="1433"/>
            </div>
            <div className="grid grid-cols-3 items-center gap-2">
              <Label htmlFor="database" className="text-xs">Database</Label>
              <Input id="database" name="database" value={formData.database} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="Database Name"/>
            </div>
            <div className="grid grid-cols-3 items-center gap-2">
              <Label htmlFor="domain" className="text-xs">Domain</Label>
              <Input id="domain" name="domain" value={formData.domain} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="Windows Domain"/>
            </div>
            {serverType === 'P21' && (<div className="grid grid-cols-3 items-center gap-2">
                <Label htmlFor="username" className="text-xs">Username</Label>
                <Input id="username" name="username" value={formData.username} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="Username"/>
              </div>)}
            <div className="grid grid-cols-3 items-center gap-2">
              <Label htmlFor="password" className="text-xs">Password</Label>
              <Input id="password" name="password" type="password" value={formData.password} onChange={handleChange} className="col-span-2 h-7 text-xs" placeholder="Password"/>
            </div>
            <DialogFooter className="flex justify-between p-4">
              ...
              <Button type="submit" variant="default" size="sm" className="h-7 text-xs" disabled={isConnecting}>
