...
import { Box, Button, Typography, CircularProgress, Tabs, Tab, List, ListItem, ListItemText, TextField, Alert } from '@mui/material';
...
function TabPanel(props) {
    ...
    return (<div role="tabpanel" hidden={value !== index} id={`tabpanel-${index}`} aria-labelledby={`tab-${index}`} {...other} style={{ padding: '20px' }}>
      {value === index && (<Box>
          {children}
        </Box>)}
    </div>);
...
export default function TestAllPORSQLPage() {
    ...
    const [successfulExpressions, setSuccessfulExpressions] = useState([]);
    const [potentialMatches, setPotentialMatches] = useState(null);
    // New state for interactive table matching
    const [currentMatching, setCurrentMatching] = useState(null);
    // State for tracking testing progress
    const [currentTestingTable, setCurrentTestingTable] = useState('');
    const [currentTestingExpression, setCurrentTestingExpression] = useState(null);
    const [tableProgress, setTableProgress] = useState({ current: 0, total: 0 });
    const [tableColumns, setTableColumns] = useState({});
    const [selectedTable, setSelectedTable] = useState('');
    // Function to try SQL expression with different tables
    const tryWithDifferentTables = (expr, tables) => __awaiter(this, void 0, void 0, function* () {
        ...
        yield tryNextTable(expr, originalTableName, tablesToTry, []);
    });
    // Function to try the next table in the list
    const tryNextTable = (expr, originalTableName, remainingTables, testedTables) => __awaiter(this, void 0, void 0, function* () {
        ...
        let testSql = expr.fixedSql.replace(new RegExp(`FROM\\s+${originalTableName.replace(/\./g, '\\.')}\\b`, 'i'), `FROM ${currentTable}`);
        // Get columns for the current table if not already cached
        if (!tableColumns[currentTable]) {
            yield getTableColumns(currentTable);
        }
        ...
        try {
            ...
            yield tryNextTable(expr, originalTableName, newRemainingTables, newTestedTables);
        }
        catch (error) {
    ...
    const getTableColumns = (tableName) => __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch('/api/get-table-columns', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!data.error && data.columns) {
                setTableColumns(prev => (Object.assign(Object.assign({}, prev), { [tableName]: data.columns })));
            }
    ...
    const acceptCurrentMatch = () => {
        ...
        setPorExpressions(prev => prev.map(expr => expr.id === currentMatching.expressionId
            ? Object.assign(Object.assign({}, expr), { fixedSql: currentMatching.currentSql }) : expr));
    ...
    const skipCurrentMatch = () => __awaiter(this, void 0, void 0, function* () {
        if (!currentMatching)
            return;
        // Try the next table
        yield tryNextTable(porExpressions.find(expr => expr.id === currentMatching.expressionId), currentMatching.originalTableName, currentMatching.remainingTables, currentMatching.testedTables);
    });
    ...
    const fixPORSqlExpression = (sql, availableTables) => {
        if (!sql)
            return sql;
        ...
        fixedSql = fixedSql.replace(/DATEADD\((\w+),\s*([^,]+),\s*([^)]+)\)/gi, (match, interval, number, date) => `DateAdd('${interval}', ${number}, ${date})`);
        fixedSql = fixedSql.replace(/DATEDIFF\((\w+),\s*([^,]+),\s*([^)]+)\)/gi, (match, interval, date1, date2) => `DateDiff('${interval}', ${date1}, ${date2})`);
        // Fix NULL handling
        fixedSql = fixedSql.replace(/ISNULL\(([^,]+),\s*([^)]+)\)/gi, (match, expr, replacement) => `Nz(${expr}, ${replacement})`);
        return fixedSql;
    };
    ...
    const sortTablesBySimilarity = (originalTable, tables) => {
    ...
    const findClosestColumnMatch = (columnName, availableColumns) => {
        if (!columnName || availableColumns.length === 0)
            return null;
        // Check for exact match first (case insensitive)
        const exactMatch = availableColumns.find(col => col.toLowerCase() === columnName.toLowerCase());
        if (exactMatch)
            return exactMatch;
    ...
    const adjustSqlColumnNames = (sql, tableColumns) => {
        if (!sql || tableColumns.length === 0)
            return sql;
        let adjustedSql = sql;
        ...
        columnRegexes.forEach(regex => {
            ...
            while ((columnMatches = columnRegex.exec(clauseContent)) !== null) {
                ...
                if (sqlKeywords.includes(originalColumn.toUpperCase()))
                    continue;
                ...
                if (closestMatch && closestMatch !== originalColumn) {
                    ...
                    adjustedClauseContent = adjustedClauseContent.replace(new RegExp(`\\b${originalColumn}\\b`, 'g'), closestMatch);
                }
            }
    ...
    const checkColumnCompatibility = (sql, tableName) => __awaiter(this, void 0, void 0, function* () {
        try {
            // Get columns for the table if not already cached
            if (!tableColumns[tableName]) {
                yield getTableColumns(tableName);
            }
            ...
            const matchingColumns = sqlColumns.filter(col => columnsLower.includes(col.toLowerCase()));
    ...
    const calculateColumnSimilarityScore = (sqlColumns, tableColumns) => {
        if (!sqlColumns.length || !tableColumns.length)
            return 0;
        let matchScore = 0;
    ...
    const sortTablesByColumnSimilarity = (sql, tables) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        ...
        if (sqlColumns.length === 0) {
            ...
            return sortTablesBySimilarity(((_a = sql.match(/FROM\s+([^\s,;()]+)/i)) === null || _a === void 0 ? void 0 : _a[1]) || '', tables);
        }
    ...
    const calculateSemanticSimilarity = (str1, str2) => {
        if (!str1 || !str2)
            return 0;
    ...
    const skipPotentialMatches = () => __awaiter(this, void 0, void 0, function* () {
        setPotentialMatches(null);
        yield testNextExpression(testingIndex + 1);
    });
    // Function to select a potential match for testing
    const selectPotentialMatch = (tableName) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        if (!potentialMatches)
            return;
        const expr = porExpressions[testingIndex];
        const originalTableName = ((_a = expr.sql.match(/FROM\s+([^\s,;()]+)/i)) === null || _a === void 0 ? void 0 : _a[1]) || '';
        // Replace the table name in the SQL
        let testSql = expr.sql.replace(new RegExp(`FROM\\s+${originalTableName.replace(/\./g, '\\.')}\\b`, 'i'), `FROM ${tableName}`);
    ...
    const testAllExpressions = () => __awaiter(this, void 0, void 0, function* () {
        ...
        setPorExpressions(prev => prev.map(expr => (Object.assign(Object.assign({}, expr), { fixedSql: expr.sql }))));
        try {
            if (porExpressions.length > 0) {
                // Start with the first expression
                yield testNextExpression(0);
            }
        }
        catch (error) {
    ...
    const testNextExpression = (index) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (!tableMatch) {
            ...
            // Continue to the next expression
            yield testNextExpression(index + 1);
            return;
        }
        // Find potential matches first
        yield findPotentialMatches(expr);
    });
            ...
            if (data.success) {
                ...
                setPorExpressions(prev => prev.map(expr => {
                    if (updatedExpressions.has(expr.id)) {
                        ...
                        if (matchingExpr) {
                            return Object.assign(Object.assign({}, expr), { sql: matchingExpr.productionSqlExpression, fixedSql: matchingExpr.productionSqlExpression });
                        }
                    }
                    return expr;
                }));
    ...
    return (<div style={{ padding: '20px' }}>
      ...
      <Box mb={4}>
        ...
        <Box mt={2}>
          ...
          <Button variant="contained" color="secondary" onClick={updateAllSqlExpressions} disabled={loading || successfulExpressions.length === 0}>
            Update All Successful Expressions
          </Button>

          {loading && (<Box mt={2} display="flex" flexDirection="column">
              ...
              {currentTestingExpression && (<Box mt={1} ml={4}>
              ...
              {currentTestingTable && (<Box mt={1} ml={4}>
                  <Typography variant="body2">
      ...
      <TabPanel value={tabValue} index={0}>
        ...
        {testResults.length === 0 ? (<p>No successful expressions yet. Run the test first.</p>) : (<div className="space-y-4">
            {testResults.map((result) => (<div key={result.id} className="border p-4 rounded bg-green-50">
                <div className="flex justify-between">
                  ...
                  <div className="flex space-x-2">
                    ...
                    {result.hasNonZeroResult ? (<span className="bg-green-500 text-white px-2 py-1 rounded text-sm">Non-Zero Result</span>) : (<span className="bg-yellow-500 text-white px-2 py-1 rounded text-sm">Zero Result</span>)}
                  </div>
                </div>
                <div className="mt-2">
                ...
                {result.originalSql !== result.sql && (<div className="mt-2">
      ...
      <TabPanel value={tabValue} index={1}>
        ...
        {testErrors.length === 0 ? (<p>No failed expressions yet. Run the test first.</p>) : (<div className="space-y-4">
            {testErrors.map((error) => (<div key={error.id} className="border p-4 rounded bg-red-50">
                <div className="flex justify-between">
                  <h3 className="font-bold">{error.dataPoint} (ID: {error.id})</h3>
                  {error.hasNonZeroResult === false && (<span className="bg-yellow-500 text-white px-2 py-1 rounded text-sm">Zero Result</span>)}
                </div>
                <div className="mt-2">
      ...
      <TabPanel value={tabValue} index={2}>
        ...
        <div className="space-y-4">
          {porExpressions.map((expr) => (<div key={expr.id} className="border p-4 rounded bg-gray-50">
        ...
        <Box display="flex">
          <Box width="30%" pr={2} sx={{ borderRight: '1px solid #ddd' }}>
            ...
            <List dense component="div" style={{ maxHeight: '500px', overflow: 'auto' }}>
              {availableTables.map((table, index) => (<ListItem key={index} onClick={() => getTableColumns(table)} sx={{
                cursor: 'pointer',
          ...
          <Box width="70%" pl={2}>
            ...
            {selectedTable && tableColumns[selectedTable] ? (<List dense component="div" style={{ maxHeight: '500px', overflow: 'auto' }}>
                {tableColumns[selectedTable].map((column, index) => (<ListItem key={index}>
                    <ListItemText primary={column}/>
                  </ListItem>))}
              </List>) : selectedTable ? (<CircularProgress size={24}/>) : null}
          </Box>
        </Box>
      </TabPanel>
      ...
      <TabPanel value={tabValue} index={5}>
        {currentMatching && (<>
            ...
            <Box mt={3}>
              <Button variant="contained" color="primary" onClick={acceptCurrentMatch} sx={{ mr: 2 }}>
                Accept Match
              </Button>
              <Button variant="contained" color="secondary" onClick={skipCurrentMatch}>
                Skip Match
              </Button>
            </Box>
          </>)}
        {!currentMatching && (<Typography>No current matching to display.</Typography>)}
      </TabPanel>
      ...
      <TabPanel value={tabValue} index={6}>
        {potentialMatches && (<>
            ...
            <Box mt={3}>
              ...
              {potentialMatches.potentialTables.map((match, index) => (<Box key={index} p={2} border="1px solid #ddd" borderRadius="4px" mb={2}>
                  ...
                  <Button variant="contained" color="primary" onClick={() => selectPotentialMatch(match.tableName)} sx={{ mt: 2 }}>
                    Try This Table
                  </Button>
                </Box>))}
              <Button variant="contained" color="secondary" onClick={skipPotentialMatches}>
                Skip All Potential Matches
              </Button>
            </Box>
          </>)}
        {!potentialMatches && (<Typography>No potential matches to display.</Typography>)}
      </TabPanel>
    </div>);
}
