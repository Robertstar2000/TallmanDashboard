...
export default function TestSQLPage() {
    ...
    const [porTables, setPorTables] = useState([]);
    const [selectedExpression, setSelectedExpression] = useState('');
    // Get POR tables on page load
    useEffect(() => {
        const fetchPorTables = () => __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield fetch('/api/test-sql');
                const data = yield response.json();
                if (data.tables) {
                    // Extract table names from the response
                    const tableNames = data.tables.map((table) => table.TableName || table.value);
    ...
    const filteredExpressions = initialSpreadsheetData.filter(item => item.serverName === serverType);
    // Execute SQL expression
    const executeSQL = () => __awaiter(this, void 0, void 0, function* () {
        if (!sqlExpression) {
      ...
      <Paper elevation={3} className="p-4 mb-4">
        <Box mb={3}>
          ...
          <FormControl fullWidth variant="outlined" className="mb-4">
            ...
            <Select value={selectedExpression} onChange={handleExpressionChange} label="Select SQL Expression">
              <MenuItem value="">-- Select an expression --</MenuItem>
              {filteredExpressions.map((expr) => (<MenuItem key={expr.id} value={expr.id}>
                  {expr.DataPoint} (ID: {expr.id})
                </MenuItem>))}
            </Select>
          </FormControl>

          {serverType === 'POR' && (<Box mb={3}>
              <Typography variant="h6">Available POR Tables:</Typography>
              <Box className="border p-2 mt-2 max-h-40 overflow-y-auto">
                {porTables.length > 0 ? (<ul>
                    {porTables.map((table, index) => (<li key={index}>{table}</li>))}
                  </ul>) : (<Typography>No tables found or still loading...</Typography>)}
              </Box>
            </Box>)}

          <TextField label="SQL Expression" variant="outlined" value={sqlExpression} onChange={(e) => setSqlExpression(e.target.value)} placeholder="Enter SQL expression" fullWidth multiline rows={4} margin="normal"/>

          <Button variant="contained" color="primary" onClick={executeSQL} disabled={loading || !sqlExpression} className="mt-4">
            {loading ? 'Executing...' : 'Execute SQL'}
          </Button>
        </Box>

        {error && (<Box className="p-4 bg-red-100 text-red-700 rounded mb-4">
            <Typography variant="h6">Error:</Typography>
            <pre>{error}</pre>
          </Box>)}

        {result && (<Box className="mt-4">
