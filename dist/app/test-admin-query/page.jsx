...
export default function TestAdminQueryPage() {
    const [serverType, setServerType] = useState('P21');
    const [tableName, setTableName] = useState('sy_param');
    const [query, setQuery] = useState("SELECT DB_NAME() as db_name");
    ...
    const [server, setServer] = useState('SQL01');
    const [database, setDatabase] = useState('P21Play');
    const [useSqlAuth, setUseSqlAuth] = useState(false); // Default to Windows auth
    const [username, setUsername] = useState('');
    const [password, setPassword] = useState('');
    const [dsn, setDsn] = useState('P21Play');
    const [porFilePath, setPorFilePath] = useState('C:\\Users\\BobM\\Desktop\\POR.MDB');
    // POR MDB specific states
    const [porMdbQuery, setPorMdbQuery] = useState("SELECT * FROM PO_HDR LIMIT 10");
    const [porMdbResult, setPorMdbResult] = useState(null);
    const [porMdbLoading, setPorMdbLoading] = useState(false);
    const [porMdbError, setPorMdbError] = useState(null);
    const [porMdbRawResponse, setPorMdbRawResponse] = useState(null);
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            if (savedUseDirect)
                setUseDirectConnection(savedUseDirect === 'true');
            if (savedUseSqlAuth)
                setUseSqlAuth(savedUseSqlAuth === 'true');
            // POR connection details
            const savedPorFilePath = localStorage.getItem('por_file_path');
            if (savedPorFilePath)
                setPorFilePath(savedPorFilePath);
        }
    }, []);
    ...
    const executeQuery = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (serverType === 'P21') {
                ...
                console.log('Using P21 config:', Object.assign(Object.assign({}, config), { password: config.password ? '***' : undefined, authentication: useSqlAuth ? 'SQL Server Authentication' : 'Windows Authentication (trusted connection)' }));
            }
            else if (serverType === 'POR') {
                config = {
                    filePath: porFilePath || process.env.POR_FILE_PATH || 'C:\\POR\\PORENT.mdb'
            ...
            setRawResponse((prev) => (Object.assign(Object.assign({}, prev), { success: false, error: err.message, errorType: err.name === 'TypeError' ? 'connection' : 'execution' })));
        }
        finally {
            setLoading(false);
        }
    ...
    const executePorMdbQuery = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const response = yield fetch('/api/por-mdb-query', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            // Make sure error information is available in the raw response
            setPorMdbRawResponse((prev) => (Object.assign(Object.assign({}, prev), { success: false, error: err.message, errorType: err.name === 'TypeError' ? 'connection' : 'execution' })));
        }
        finally {
      ...
      <Paper elevation={3} className="p-4 mb-4">
        <Box mb={3}>
          <FormControl fullWidth variant="outlined" className="mb-4">
            ...
            <Select value={serverType} onChange={(e) => setServerType(e.target.value)} label="Server Type">
          ...
          <FormControl fullWidth variant="outlined" className="mb-4">
            <TextField label="Table Name" variant="outlined" value={tableName} onChange={(e) => setTableName(e.target.value)} placeholder="Enter table name (optional)" fullWidth margin="normal"/>
          </FormControl>

          {serverType === 'P21' && (<>
              <FormControl fullWidth variant="outlined" className="mb-4">
                <InputLabel id="direct-connection-label">Use Direct Connection</InputLabel>
                <Select labelId="direct-connection-label" value={useDirectConnection ? "true" : "false"} onChange={(e) => setUseDirectConnection(e.target.value === 'true')} label="Use Direct Connection">
                  <MenuItem value="true">Yes</MenuItem>
                  <MenuItem value="false">No</MenuItem>
                </Select>
              </FormControl>

              {!useDirectConnection && (<Box className="p-4 border border-gray-300 rounded mb-4">
                  <Typography variant="h6" gutterBottom>ODBC Connection Details</Typography>
                  
                  <TextField label="DSN Name" variant="outlined" value={dsn} onChange={(e) => setDsn(e.target.value)} placeholder="Enter DSN name" fullWidth margin="normal"/>
                  
                  <TextField label="Database Name" variant="outlined" value={database} onChange={(e) => setDatabase(e.target.value)} placeholder="Enter database name (e.g., P21)" fullWidth margin="normal"/>
                  ...
                  <FormControl fullWidth variant="outlined" className="mb-4 mt-4">
                    ...
                    <Select labelId="sql-auth-label" value={useSqlAuth ? "true" : "false"} onChange={(e) => setUseSqlAuth(e.target.value === 'true')} label="Use SQL Server Authentication">
                      <MenuItem value="true">Yes</MenuItem>
                  ...
                  {useSqlAuth && (<>
                      <TextField label="Username" variant="outlined" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter SQL Server username" fullWidth margin="normal"/>
                      
                      <TextField label="Password" variant="outlined" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter SQL Server password" fullWidth margin="normal" type="password"/>
                    </>)}
                </Box>)}
              ...
              {useDirectConnection && (<Box className="p-4 border border-gray-300 rounded mb-4">
                  ...
                  <TextField label="Server" variant="outlined" value={server} onChange={(e) => setServer(e.target.value)} placeholder="Enter server" fullWidth margin="normal"/>
                  
                  <TextField label="Database" variant="outlined" value={database} onChange={(e) => setDatabase(e.target.value)} placeholder="Enter database" fullWidth margin="normal"/>
          ...
          {serverType !== 'P21' && (<FormControl fullWidth variant="outlined" className="mb-4">
              <TextField label="File Path" variant="outlined" value={porFilePath} onChange={(e) => setPorFilePath(e.target.value)} placeholder="Enter file path" fullWidth margin="normal"/>
            </FormControl>)}
          <Button variant="contained" color="secondary" onClick={testConnection} disabled={loading} className="mb-4">
            {loading ? <CircularProgress size={24}/> : 'Test Connection'}
          </Button>

          {connectionStatus && connectionStatus !== 'unknown' && (<Alert severity={connectionStatus.includes('successful') ? "success" : "error"} className="mb-4">
              {connectionStatus}
            </Alert>)}
        ...
        <TextField label="SQL Query" variant="outlined" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Enter your SQL query" fullWidth multiline rows={4} margin="normal"/>

        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <Button variant="contained" color="primary" onClick={executeQuery} disabled={loading || !query} className="mr-2">
            {loading ? <CircularProgress size={24}/> : 'Execute Query'}
          </Button>
          
          <Button variant="outlined" color="primary" onClick={tryAlternativeQuery} disabled={loading || !query}>
            Try Alternative Query
          </Button>
        </Box>

        {serverType === 'P21' && (<Box className="mt-4">
...
SELECT TOP 5 * FROM system_parameters;
      ...
      <Paper elevation={3} className="p-4 mb-4">
        ...
        <Box mb={3} display="flex" alignItems="flex-start">
          <TextField label="POR MDB File Path" value={porFilePath} onChange={(e) => {
            setPorFilePath(e.target.value);
            localStorage.setItem('por_file_path', e.target.value);
        }} fullWidth margin="normal" variant="outlined" helperText="Path to the POR MDB file (e.g., C:\Users\BobM\Desktop\POR.MDB)" style={{ marginRight: '16px' }}/>
        ...
        <Box mb={3}>
          <TextField label="POR MDB Query" value={porMdbQuery} onChange={(e) => setPorMdbQuery(e.target.value)} fullWidth multiline rows={4} margin="normal" variant="outlined" helperText="Enter a query to execute against the POR MDB file (e.g., SELECT * FROM PO_HDR LIMIT 10, SHOW TABLES, DESCRIBE PO_HDR)"/>
        </Box>

        <Box mb={3} display="flex" justifyContent="flex-start">
          <Button variant="contained" color="primary" onClick={executePorMdbQuery} disabled={porMdbLoading} style={{ marginRight: '8px' }}>
        ...
        <Box className="mt-4 mb-4">
          ...
          <Box display="flex" flexWrap="wrap" gap={2}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 2, width: '100%' }}>
              <TextField label="Table Name" variant="outlined" size="small" value={customTableName} onChange={(e) => setCustomTableName(e.target.value)} sx={{ flexGrow: 1, maxWidth: 200 }}/>
        ...
        {porMdbError && (<Alert severity="error" className="mb-4">{porMdbError}</Alert>)}

        {porMdbRawResponse && (<Paper elevation={3} className="p-4">
        ...
        {porMdbRawResponse && porMdbRawResponse.error && (<Paper elevation={3} className="p-4 mt-4">
            ...
            <Alert severity="error" className="mb-2">
              ...
              {porMdbRawResponse.errorType && (<Typography variant="subtitle2"><strong>Error Type:</strong> {porMdbRawResponse.errorType}</Typography>)}
            </Alert>
          </Paper>)}

        {porMdbResult && (<Paper elevation={3} className="p-4">
            <Typography variant="h6" gutterBottom>POR MDB Query Result</Typography>
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border">
                <thead className="bg-gray-100">
                  <tr>
                    {porMdbResult.length > 0 && Object.keys(porMdbResult[0]).map((key) => (<th key={key} className="py-2 px-4 border">{key}</th>))}
                  </tr>
                </thead>
                <tbody>
                  {porMdbResult.map((row, rowIndex) => (<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-gray-50' : ''}>
                      {Object.values(row).map((value, colIndex) => (<td key={colIndex} className="py-2 px-4 border">
                          {value === null ? 'NULL' :
      ...
      {rawResponse && (<Paper elevation={3} className="p-4">
      ...
      {rawResponse && rawResponse.error && (<Paper elevation={3} className="p-4 mt-4">
          ...
          <Alert severity="error" className="mb-2">
            ...
            {rawResponse.errorType && (<Typography variant="subtitle2"><strong>Error Type:</strong> {rawResponse.errorType}</Typography>)}
          </Alert>
        </Paper>)}
      
      {connectionDetails && (<Paper elevation={3} className="p-4">
