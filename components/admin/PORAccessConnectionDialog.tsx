...
export default function PORAccessConnectionDialog() {
  ...
  useEffect(() => {
    ...
    if (savedPath) {
      ...
      // Default path if none is saved
      setFilePath('C:\\Users\\BobM\\Desktop\\POR.MDB');
    }
  }, []);
  ...
  // Validate file path
  const validateFilePath = (path: string): { valid: boolean; message?: string } => {
    if (!path) {
      return { valid: false, message: 'File path is required' };
    }

    // Check if path ends with .mdb or .accdb
    if (!path.toLowerCase().endsWith('.mdb') && !path.toLowerCase().endsWith('.accdb')) {
      return { valid: false, message: 'File path must end with .mdb or .accdb extension' };
    }
