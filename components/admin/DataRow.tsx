...
export function DataRow({
  ...
  return (
    <div className={`grid grid-cols-5 gap-4 p-4 ${isCurrentRow ? 'bg-blue-50' : ''}`}>
      {/* Value Field */}
      <div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) => onSqlChange(row, e.target.value)}
          disabled={isProcessing}
      ...
      <div>
        ...
        <Input
          id={`table-${row.id}`}
          value={row.tableName || ''}
          onChange={(e) => onTableNameChange(row, e.target.value)}
          disabled={isProcessing}
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => onServerNameChange(row, e.target.value)}
          disabled={isProcessing}
