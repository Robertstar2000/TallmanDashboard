...
/**
 * Script to update the Daily Orders SQL expressions in the SQLite database
 */
async function updateDailyOrdersQueries() {
  ...
  try {
    ...
    for (const row of dailyOrdersRows) {
      ...
      // Create SQL expression for counting orders on a specific day
      const sqlExpression = `SELECT COUNT(*) as value FROM order_hdr WHERE CONVERT(date, order_date) = CONVERT(date, DATEADD(day, -${dayOffset}, GETDATE()))`;
      const tableName = 'order_hdr';
