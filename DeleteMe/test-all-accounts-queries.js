...
// Output file path
const outputFilePath = path.join(__dirname, 'all-accounts-queries-results.txt');
...
async function testAllAccountsQueries() {
  // Clear the output file if it exists
  if (fs.existsSync(outputFilePath)) {
    fs.unlinkSync(outputFilePath);
  }

  writeToFile('=== P21 All Accounts Queries Test ===');
  writeToFile(`Starting test at ${new Date().toISOString()}`);

  // Define all queries to test
  const queriesToTest = [
    // Accounts - Payable - All Months
    {
      name: "Accounts - Payable - Month 1",
      tableName: "inv_mast",
      sqlQuery: "SELECT ISNULL(COUNT(*)/12, 0) AS value FROM dbo.inv_mast WITH (NOLOCK) WHERE delete_flag <> 'Y'"
    },
    {
