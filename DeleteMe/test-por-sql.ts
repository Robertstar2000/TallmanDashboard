...
async function main() {
  ...
  let alternativeTables: string[] = [];
  
  if (!hasRentalsTable) {
    console.log('\nLooking for alternative tables...');
    
    for (const pattern of ALTERNATIVE_TABLE_PATTERNS) {
      const matches = availableTables.filter(table => 
        table.toLowerCase().includes(pattern.pattern.toLowerCase())
      );
      ...
      if (matches.length > 0) {
        ...
        // Try the fallback
        const fallbackMatches = availableTables.filter(table => 
          table.toLowerCase().includes(pattern.fallback.toLowerCase())
        );
