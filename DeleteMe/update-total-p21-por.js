...
// Find all the Total {P21+POR} entries and update their SQL expressions
let updatedContent = content.replace(
  /variableName: ['"]Total \{P21\+POR\}['"][\s\S]*?productionSqlExpression: [`'"].*?[`'"]/g,
  (match) => {
...
const count = (updatedContent.match(/Total \{P21\+POR\}.*?productionSqlExpression: "SELECT \(SELECT COALESCE/g) || []).length;
