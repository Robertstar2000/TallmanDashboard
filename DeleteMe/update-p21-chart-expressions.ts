...
async function testP21Table(tableName: string): Promise<boolean> {
...
async function getP21TableColumns(tableName: string): Promise<string[]> {
...
function extractTableName(sqlExpression: string): string | null {
...
function extractColumnNames(sqlExpression: string): string[] {
...
async function updateP21ChartExpressions() {
  ...
  const chartGroups = new Map<string, SpreadsheetRow[]>();
  ...
  for (const [chartGroup, entries] of chartGroups.entries()) {
    ...
    const continue_ = await question(`Process chart group '${chartGroup}'? (Y/N): `);
    ...
    for (const entry of p21Entries) {
      ...
      const currentTable = extractTableName(entry.productionSqlExpression);
      ...
      if (currentTable) {
        ...
        const tableExists = await testP21Table(tableName);
        
        if (tableExists) {
          console.log(`Table ${tableName} exists in P21`);
          
          // Get the columns for the table
          const columns = await getP21TableColumns(tableName);
          console.log(`Columns for table ${tableName}: ${columns.join(', ')}`);
          
          // Extract column names from the SQL expression
          const sqlColumns = extractColumnNames(entry.productionSqlExpression);
          ...
          if (missingColumns.length > 0) {
            ...
            for (const missingCol of missingColumns) {
              const newColumn = await question(`Enter a replacement column for '${missingCol}' (or press Enter to skip): `);
              
              if (newColumn && newColumn !== missingCol) {
                // Update the SQL expression with the new column name
                entry.productionSqlExpression = entry.productionSqlExpression.replace(
                  new RegExp(`\\b${missingCol}\\b`, 'g'), 
                  newColumn
                );
          ...
          const newTableName = await question(`Enter a new table name for entry ID ${entry.id} (or press Enter to skip): `);
