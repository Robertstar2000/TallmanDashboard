...
async function main() {
  try {
    ...
    for (let i = 0; i < porExpressions.length; i++) {
      ...
      console.log(`Original SQL: ${expression.productionSqlExpression}`);

      // Test the original SQL
      const originalResult = await testSqlExpression(expression.productionSqlExpression);
      if (originalResult.success && !originalResult.error) {
        ...
        results.push({
          ...
          originalSql: expression.productionSqlExpression,
          fixed: false,
          working: true,
          newSql: expression.productionSqlExpression,
      ...
      const extractedTable = extractTableFromSql(expression.productionSqlExpression);
      console.log(`Extracted table: ${extractedTable}`);
      
      // Extract columns from the original SQL
      const extractedColumns = extractColumnsFromSql(expression.productionSqlExpression);
      ...
      if (extractedTable && availableTables.includes(extractedTable)) {
        ...
        const sql = fixSqlExpression(expression.productionSqlExpression, extractedTable, extractedColumns);
      ...
      if (!fixed) {
        ...
        for (const table of similarTables) {
          ...
          const sql = fixSqlExpression(expression.productionSqlExpression, table, extractedColumns);
          console.log(`Generated SQL: ${sql}`);
      ...
      if (fixed) {
        ...
        console.log(`Original: ${expression.productionSqlExpression}`);
