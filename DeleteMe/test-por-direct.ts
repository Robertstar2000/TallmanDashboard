...
async function main() {
  ...
  try {
    ...
    for (const test of TEST_QUERIES) {
      ...
      const response = await testSql(test.query);
    ...
    for (const query of rentalQueries) {
      ...
      const response = await testSql(query);
    ...
    console.log('\nMethod 1: Using SHOW TABLES');
    const showTablesResponse = await testSql('SHOW TABLES');
    ...
    console.log('\nMethod 2: Using INFORMATION_SCHEMA');
    const infoSchemaResponse = await testSql('SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES');
    ...
    console.log('\nMethod 3: Using MSysObjects (Access specific)');
    const msysObjectsResponse = await testSql("SELECT Name FROM MSysObjects WHERE Type=1 AND Flags=0");
