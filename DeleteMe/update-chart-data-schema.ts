 ...
 * - production_sql_expression: Query to execute in production environment
...
async function updateChartDataSchema() {
  try {
    ...
    if (!tableExists) {
      ...
      const correctSchema = [
        ...
        { name: 'production_sql_expression', type: 'TEXT' },
      ...
      const needsUpdate = columns.some(col => {
        ...
        if (col.name === 'serverName') return true;
        // Check if column name is 'db_table_name' instead of 'table_name'
        if (col.name === 'db_table_name') return true;
        return false;
      });
      
      // Check for missing columns
      const missingColumns = correctSchema.filter(correctCol => 
        !columns.some(col => col.name === correctCol.name)
      );
      ...
      if (needsUpdate || missingColumns.length > 0) {
        ...
        const columnMap: Record<string, string> = {
          ...
          'sql_expression': 'production_sql_expression', // Map sql_expression to production_sql_expression
          'production_sql_expression': 'production_sql_expression',
          'value': 'value',
        ...
        await db.exec(`
          ...
          SELECT 
            ...
            ${columns.some(col => col.name === 'production_sql_expression') ? 'production_sql_expression' : 
              columns.some(col => col.name === 'sql_expression') ? 'sql_expression' : 'NULL'} AS production_sql_expression,
            value,
            last_updated,
      ...
      for (const missingCol of missingColumns) {
        ...
        await db.exec(`ALTER TABLE chart_data ADD COLUMN ${missingCol.name} ${missingCol.type};`);
        console.log(`Successfully added column ${missingCol.name}.`);
      }
    }
    
    // Check the updated schema
    const updatedColumns = await db.all(`PRAGMA table_info(chart_data)`);
