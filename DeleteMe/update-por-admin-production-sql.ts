 ...
 * for the POR Overview chart. It focuses on updating the production_sql_expression
...
async function updatePORAdminProductionSQL() {
  ...
  try {
    ...
    // Load and ensure all rows have productionSqlExpression
    const rawData = JSON.parse(fs.readFileSync(queriesPath, 'utf8'));
    const porOverviewRows: POROverviewRow[] = rawData.map((row: any) => ({
      ...row,
    ...
    const currentPORRows = await executeRead(sql);
    ...
    if (!Array.isArray(currentPORRows) || currentPORRows.length === 0) {
      ...
      for (const row of porOverviewRows) {
        ...
        const msAccessSQL = formatSQLForMSAccess(row.productionSqlExpression);
        ...
        const result = await executeWrite(insertSql, [
          row.variableName,
          row.value.toString(),
          'POR',
          'POR Overview',
      ...
      for (const row of porOverviewRows) {
        ...
        const adminRow = currentPORRows.find(r => 
          r.variable_name === row.variableName && 
        ...
        const result = await executeWrite(updateSql, [msAccessSQL, adminRow.id]);
    ...
    for (const row of porOverviewRows) {
      ...
      logContent += '```sql\n';
      logContent += formatSQLForMSAccess(row.productionSqlExpression);
      logContent += '\n```\n\n';
    }
 ...
 */
function formatSQLForMSAccess(sql: string): string {
  if (!sql) return '';
