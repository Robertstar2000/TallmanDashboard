...
async function main() {
  ...
  try {
    ...
    const tables = tablesResponse.result.map((row: any) => row.TableName || row.value);
    ...
    const rentalTables = tableInfos.filter(table => 
      table.name.toLowerCase().includes('rent') || 
    ...
    const tablesWithDateColumns = rentalTables.filter(table => 
    ...
    const tablesWithStatusColumns = rentalTables.filter(table => 
      table.columns.some(col => 
    ...
    const tablesWithDateAndStatus = rentalTables.filter(table => 
      table.columns.some(col => col.toLowerCase().includes('date') || col.toLowerCase().includes('time')) &&
    ...
    if (bestTable) {
      ...
      // Find date column
      const dateColumn = bestTable.columns.find(col => 
        col.toLowerCase().includes('date') || 
        col.toLowerCase().includes('time') || 
        col.toLowerCase().includes('created')
      ) || 'Date';
      ...
      // Find status column
      const statusColumn = bestTable.columns.find(col => 
        col.toLowerCase().includes('status') || 
        col.toLowerCase().includes('state') || 
        col.toLowerCase().includes('type')
      );
      ...
      for (const month of months) {
        ...
        const valueColumn = bestTable.columns.find(col => 
    ...
    for (let i = 0; i < sqlExpressions.length; i++) {
      ...
      if (response.success) {
        ...
        if (response.error && response.error.includes('not found')) {
          ...
          const alternativeResponse = await testSql(alternativeSql);
