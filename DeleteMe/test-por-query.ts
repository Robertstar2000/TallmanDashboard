  ...
  try {
    ...
    if (result && Array.isArray(result) && result.length > 0) {
      ...
      if (columns.length > 0) {
        ...
        const complexResponse = await fetch('http://localhost:3000/api/executeAccessQuery', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
 ...
 */
async function executeDirectMDBQuery() {
...
async function main() {
  ...
  // Test the connection
  const connectionSuccess = await testPORConnection();
  ...
  const directQuerySuccess = await executeDirectMDBQuery();
  ...
  try {
    ...
    const querySuccess = await executeTestQuery();
