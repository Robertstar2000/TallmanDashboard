...
const SQL_PATTERNS = [
  // New Rentals patterns
  {
    ...
    patterns: [
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Month(CreatedDate) = ${month} AND Year(CreatedDate) = Year(Date())`,
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Month(Date) = ${month} AND Year(Date) = Year(Date())`,
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Source = 'Web' AND Month(CreatedDate) = ${month} AND Year(CreatedDate) = Year(Date())`,
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Status = 'Active' AND Month(CreatedDate) = ${month} AND Year(CreatedDate) = Year(Date())`
    ]
  },
  ...
  // Open Rentals patterns
  {
    ...
    patterns: [
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Status = 'Active'`,
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Status = 'Open'`,
      (table: string, month: number) => 
        `SELECT Count(*) AS value FROM ${table} WHERE Status = 'Active' AND Month(CreatedDate) = ${month} AND Year(CreatedDate) = Year(Date())`
    ]
  },
...
async function main() {
  ...
  try {
    ...
    if (nonZeroResults.length > 0) {
      ...
      const byMetricAndMonth: Record<string, Record<string, TestResult[]>> = {};
