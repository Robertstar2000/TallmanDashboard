...
function verifyChanges() {
  ...
  db.all("SELECT id, variable_name, sql_expression, production_sql_expression, db_table_name FROM chart_data WHERE chart_group = 'AR Aging'", (err, rows) => {
    if (err) {
    ...
    // Now let's update the initial-data.ts file
    updateInitialData(rows);
  });
}

// Update the initial-data.ts file
function updateInitialData(rows) {
  ...
  fs.readFile(initialDataPath, 'utf8', (err, data) => {
    ...
    rows.forEach(row => {
      ...
      const rowRegex = new RegExp(`{\\s*id:\\s*['"]${row.id}['"].*?chartGroup:\\s*['"]AR Aging['"].*?variableName:\\s*['"]${row.variable_name}['"].*?}`, 's');
      const rowMatch = updatedData.match(rowRegex);
      
      if (rowMatch) {
        let rowData = rowMatch[0];
        
        // Update the SQL expressions and table name
        rowData = rowData.replace(/sqlExpression:\s*".*?"/, `sqlExpression: "${row.sql_expression}"`);
        rowData = rowData.replace(/productionSqlExpression:\s*".*?"/, `productionSqlExpression: "${row.production_sql_expression}"`);
        rowData = rowData.replace(/tableName:\s*".*?"/, `tableName: "${row.db_table_name}"`);
        
        // Replace the row in the file
        updatedData = updatedData.replace(rowRegex, rowData);
      } else {
        console.log(`Could not find row ${row.id} in initial-data.ts`);
      }
    });
