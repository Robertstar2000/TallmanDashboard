...
async function main() {
  try {
    ...
    for (let i = 0; i < testExpressions.length; i++) {
      ...
      console.log(`SQL: ${expression.productionSqlExpression}`);
      ...
      try {
        ...
        results.push({
          ...
          sql: expression.productionSqlExpression,
 ...
 */
async function testSqlExpression(sql: string): Promise<{ success: boolean; value?: any; error?: string }> {
  try {
 ...
 */
async function saveResults(results: SqlTestResult[]): Promise<void> {
  try {
