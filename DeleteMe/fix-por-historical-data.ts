/**
 * Fix POR Historical Data
 * 
 * This script creates or updates the POR Historical Data rows in the admin database
 * with properly formatted MS Access SQL queries.
 */
...
async function fixPorHistoricalData() {
  try {
    console.log('Fixing POR Historical Data...');
    ...
    console.log('\nPOR Historical Data fix complete');
  } catch (error) {
    console.error('Error fixing POR Historical Data:', error instanceof Error ? error.message : String(error));
  }
}

// Run the fix
fixPorHistoricalData().catch(error => {
