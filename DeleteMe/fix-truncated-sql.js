...
for (let i = 1950; i < 1970; i++) {
  if (lines[i] && lines[i].includes('productionSqlExpression: \'SELECT COALESCE(SUM(QTY_ON_HAND), 0) as value')) {
    lineIndex = i;
    break;
  }
}
...
if (lineIndex !== -1) {
  ...
  lines[lineIndex] = '    productionSqlExpression: \'SELECT COALESCE(SUM(QTY_ON_HAND), 0) as value FROM INV_MAST im JOIN PRODUCT_LINE pl ON im.PROD_LINE = pl.PROD_LINE WHERE pl.DESCRIPTION LIKE "%Plumbing%"\',';
...
const truncatedPattern = /productionSqlExpression: ['"`].*?(?<!['"`])(?=\s*,|\s*\n)/g;
