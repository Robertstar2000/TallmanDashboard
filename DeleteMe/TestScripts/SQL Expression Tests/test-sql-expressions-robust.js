...
// Function to parse CSV with custom handling for the specific format
function parseCSV(filePath) {
  try {
    ...
    const serverTypeIndex = header.findIndex(col => col.trim() === 'ServerType');
    const sqlExpressionIndex = header.findIndex(col => col.trim() === 'SqlExpression');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      for (let j = 0; j < line.length; j++) {
        if (line[j] === '"') {
          ...
          fields.push(line.substring(fieldStart, j).trim());
          fieldStart = j + 1;
        }
      }
      
      // Add the last field
      fields.push(line.substring(fieldStart).trim());
      
      // Extract required fields
      const id = fields[idIndex] || '';
      const name = fields[nameIndex] || '';
      const serverType = fields[serverTypeIndex] || '';
      const sqlExpression = fields[sqlExpressionIndex] || '';
      
      // Remove quotes from fields if present
      const cleanId = id.replace(/^"(.*)"$/, '$1');
      const cleanName = name.replace(/^"(.*)"$/, '$1');
      const cleanServerType = serverType.replace(/^"(.*)"$/, '$1');
      const cleanSqlExpression = sqlExpression.replace(/^"(.*)"$/, '$1');
      
      // Add to results if we have a server type and SQL expression
      if (cleanServerType && cleanSqlExpression) {
        results.push({
          id: cleanId,
          name: cleanName,
          serverType: cleanServerType,
          sqlExpression: cleanSqlExpression
        });
      }
    }
...
async function main() {
  ...
  const sqlExpressions = parseCSV(CSV_FILE_PATH);
