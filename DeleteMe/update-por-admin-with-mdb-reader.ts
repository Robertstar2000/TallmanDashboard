...
async function updatePORAdminWithMDBReader() {
  ...
  try {
    ...
    const currentPORRows = await executeWrite(sql);
    ...
    if (!Array.isArray(currentPORRows) || currentPORRows.length === 0) {
      ...
      for (const row of porOverviewRows) {
        ...
        await executeWrite(updateSql, [msAccessSQL, adminRow.id]);
        updatedCount++;
      }
...
function formatSQLForMSAccess(sql: string): string {
  ...
  formattedSql = formattedSql.replace(/DATEADD\((\w+),\s*([^,]+),\s*([^)]+)\)/gi, 
    (match, interval, number, date) => {
