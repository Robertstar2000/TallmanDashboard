...
async function updateAdminSpreadsheet() {
  ...
  try {
    ...
    const queriesData: AdminRow[] = rawData.map((row: any) => ({
      ...row,
    ...
    for (const [groupName, queries] of Object.entries(queryGroups)) {
      ...
      for (const query of queries) {
        ...
        const formattedSql = query.productionSqlExpression.replace(/\n\s+/g, ' ');
    ...
    for (const [groupName, queries] of Object.entries(queryGroups)) {
      ...
      for (const query of queries) {
        ...
        sqlScript += `${query.productionSqlExpression};\n\n`;
      }
    }
