...
// Initialize MCP controller for direct server communication
const mcpController = new MCPController();

// Initialize background worker with MCP integration
const backgroundWorker = new BackgroundWorker();

// Set up worker callbacks for real-time updates
let latestStatus = 'Initializing MCP connections...';
let latestMetrics = [];

// MCP Status Check Function
...
backgroundWorker.setCallbacks(
    ...
    (metrics) => {
        ...
        console.log(`Updated ${metrics.length} metrics from MCP servers`);
    }
);
...
// Execute SQL query via MCP servers
app.post('/api/query', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`📊 Executing MCP query on ${database}: ${query.substring(0, 100)}...`);
    
    // Execute query via MCP controller
    const result = await mcpController.executeQuery(database.toUpperCase(), query);
    
    console.log(`✅ MCP query completed successfully`);
    ...
    res.status(500).json({ 
      error: 'MCP query execution failed',
...
// Get dashboard data from MCP servers
app.get('/api/dashboard/data', authenticateToken, async (req, res) => {
  try {
    ...
    // Filter and structure data according to user requirements:
    // - 6 inventory metrics (2 per site: Columbus, Addison, Lake City)
    // - Historical data from POR server only (last 12 months, 2 variables)
    // - Site distribution limited to 3 sites only
    // - Clean accounts chart data
    // - NO additional key metrics that shouldn't exist
    
    const filteredMetrics = metrics.filter(metric => {
      // Only include metrics that match the required structure
      const allowedChartGroups = [
        'KEY_METRICS',
        'ACCOUNTS', 
        'INVENTORY',
        'HISTORICAL_DATA',
    ...
    // Return empty array so charts display zero values as requested
    res.json([]);
  }
});
