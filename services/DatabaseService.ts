...
class DatabaseService {
  ...
  getAllMetrics(): DashboardMetric[] {
    ...
    while (stmt.step()) {
      ...
      results.push(row as unknown as DashboardMetric);
    }
    stmt.free();
  ...
  getMetricsByChartGroup(chartGroup: string): DashboardMetric[] {
    ...
    stmt.bind({ $chartGroup: chartGroup });
    const results: DashboardMetric[] = [];
    while (stmt.step()) {
      const row = stmt.getAsObject();
      results.push(row as unknown as DashboardMetric);
  ...
  async authenticateUser(userName: string): Promise<AuthenticatedUser | null> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  getAllUsers(): AuthenticatedUser[] {
    ...
    while (stmt.step()) {
      const row = stmt.getAsObject();
      results.push(row as unknown as AuthenticatedUser);
    }
    stmt.free();
