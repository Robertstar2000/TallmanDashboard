...
export default function TestP21Page() {
  ...
  const runTest = async () => {
    ...
    try {
      const response = await fetch('/api/test-p21');
      ...
      {results.length > 0 && (
        <div className="mt-4">
          ...
          {results.map((result, index) => (
            <div key={index} className={`mb-4 p-4 rounded ${result.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
