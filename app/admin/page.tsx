...
export default function AdminPage() {
  ...
  React.useEffect(() => {
    const updateInterval = setInterval(() => {
      ...
      const porData = getPOR();
      const inventoryData = getInventoryValue();
      const webData = getWebMetrics();
      const historicalData = getHistoricalData();

      // Update chart data with latest values
      setChartData(prev => prev.map(row => {
        // Find the corresponding data based on chart name and variable name
        let newValue = row.value;

        switch(row.chartName) {
          case 'Metrics':
            const metric = metricsData.find(m => m.name === row.variableName);
            if (metric) newValue = metric.value;
            break;
          case 'Daily Orders':
            const order = dailyOrdersData.find(d => d.day === row.variableName);
          ...
          case 'Customer Metrics':
            const customer = customerData.find(c => c.month === row.x_axis);
            if (customer) newValue = customer[row.variableName as keyof typeof customer];
            break;
          case 'Accounts':
            const account = accountsData.find(a => a.month === row.x_axis);
            if (account) newValue = account[row.variableName as keyof typeof account];
            break;
          case 'POR':
            const por = porData.find(p => p.month === row.x_axis);
            if (por) newValue = por[row.variableName as keyof typeof por];
            break;
          case 'Inventory':
            const inventory = inventoryData.find(i => i.category === row.variableName);
            if (inventory) newValue = inventory[row.variableName as keyof typeof inventory];
            break;
          case 'Web Metrics':
            const web = webData.find(w => w.month === row.x_axis);
            if (web) newValue = web[row.variableName as keyof typeof web];
            break;
          case 'Historical Data':
            const historical = historicalData.find(h => h.month === row.x_axis);
            if (historical) newValue = historical[row.variableName as keyof typeof historical];
            break;
        }
  ...
  const getP21SqlInfo = (chartName: string, variableName: string, relativeDate: string) => {
    switch (chartName) {
      case 'Metrics':
        return {
          sql: 'SELECT COUNT(*) FROM order_header WHERE CONVERT(date, order_date) = CONVERT(date, GETDATE())',
          table: 'order_header'
        };
      case 'Daily Orders':
        return {
          sql: `SELECT COUNT(*) FROM order_header WHERE CONVERT(date, order_date) = CONVERT(date, '${relativeDate}')`,
          table: 'order_header'
        };
      case 'Site Distribution':
        return {
          sql: 'SELECT location_id, COUNT(*) as value FROM order_header GROUP BY location_id',
          table: 'order_header'
        };
      case 'Customer Metrics':
        if (variableName === 'newCustomers') {
          return {
            sql: `SELECT COUNT(*) FROM customer WHERE CONVERT(varchar(7), creation_date, 120) = '${relativeDate}' AND customer_type = 'CUSTOMER'`,
            table: 'customer'
          };
        }
        return {
          sql: `SELECT COUNT(*) FROM customer WHERE CONVERT(varchar(7), creation_date, 120) = '${relativeDate}' AND customer_type = 'PROSPECT'`,
          table: 'customer'
        };
      case 'Accounts':
        if (variableName === 'payable') {
          return {
            sql: `SELECT SUM(amount_due) FROM ap_header WHERE CONVERT(varchar(7), invoice_date, 120) = '${relativeDate}'`,
            table: 'ap_header'
          };
        }
        if (variableName === 'overdue') {
          return {
            sql: `SELECT SUM(amount_due) FROM ap_header WHERE CONVERT(varchar(7), invoice_date, 120) = '${relativeDate}' AND DATEDIFF(day, due_date, GETDATE()) > 30`,
            table: 'ap_header'
          };
        }
        return {
          sql: `SELECT SUM(amount_due) FROM ar_header WHERE CONVERT(varchar(7), invoice_date, 120) = '${relativeDate}'`,
          table: 'ar_header'
        };
      case 'Inventory':
        if (variableName === 'inStock') {
          return {
            sql: `SELECT SUM(qty_on_hand) FROM item_warehouse WHERE category = '${relativeDate}'`,
            table: 'item_warehouse'
          };
        }
        return {
          sql: `SELECT SUM(qty_ordered) FROM po_detail pd JOIN po_header ph ON pd.po_no = ph.po_no JOIN inventory i ON pd.item_id = i.item_id WHERE i.category = '${relativeDate}' AND ph.status = 'OPEN'`,
          table: 'po_detail, po_header, inventory'
        };
      case 'Web Metrics':
        if (variableName === 'W_Orders') {
          return {
            sql: `SELECT COUNT(*) FROM order_header WHERE CONVERT(varchar(7), order_date, 120) = '${relativeDate}' AND order_source = 'WEB'`,
            table: 'order_header'
          };
        }
        return {
          sql: `SELECT SUM(extended_amount) FROM order_line ol JOIN order_header oh ON ol.order_no = oh.order_no WHERE CONVERT(varchar(7), oh.order_date, 120) = '${relativeDate}' AND oh.order_source = 'WEB'`,
          table: 'order_line, order_header'
        };
      case 'Historical Data':
        if (variableName === 'p21') {
          return {
            sql: `SELECT SUM(extended_amount) FROM invoice_line il JOIN invoice_header ih ON il.invoice_no = ih.invoice_no WHERE CONVERT(varchar(7), ih.invoice_date, 120) = '${relativeDate}'`,
            table: 'invoice_line, invoice_header'
          };
        }
  ...
  const getPORSqlInfo = (chartName: string, variableName: string, relativeDate: string) => {
    switch (chartName) {
      case 'POR':
        if (variableName === 'newRentals') {
          return {
            sql: `SELECT COUNT(*) FROM tblContracts WHERE CONVERT(varchar(7), ContractDate, 120) = '${relativeDate}' AND ContractStatus = 'Out'`,
            table: 'tblContracts'
          };
        }
        if (variableName === 'openRentals') {
          return {
            sql: `SELECT COUNT(*) FROM tblContracts WHERE CONVERT(varchar(7), ContractDate, 120) = '${relativeDate}' AND ContractStatus NOT IN ('Closed', 'Cancelled')`,
            table: 'tblContracts'
          };
        }
        return {
          sql: `SELECT SUM(TotalAmount) FROM tblContracts WHERE CONVERT(varchar(7), ContractDate, 120) = '${relativeDate}'`,
          table: 'tblContracts'
        };
      case 'Historical Data':
        if (variableName === 'por') {
          return {
            sql: `SELECT SUM(TotalAmount) FROM tblContracts WHERE CONVERT(varchar(7), ContractDate, 120) = '${relativeDate}' AND ContractStatus NOT IN ('Cancelled')`,
            table: 'tblContracts'
          };
        }
        return { sql: '', table: '' };
  ...
  // Initialize chart data function
  const initializeChartData = () => {
    const tempData: Omit<ChartData, 'id'>[] = [];
    ...
    const siteData = getSiteDistribution();
    ...
    customerData.forEach(customer => {
      const sqlInfo1 = getP21SqlInfo('Customer Metrics', 'newCustomers', customer.month);
      tempData.push({
        ...
        serverName: 'P21',
        sqlExpression: sqlInfo1.sql,
        tableName: sqlInfo1.table
      });
      const sqlInfo2 = getP21SqlInfo('Customer Metrics', 'prospects', customer.month);
      tempData.push({
        chartName: 'Customer Metrics',
        ...
        serverName: 'P21',
        sqlExpression: sqlInfo2.sql,
        tableName: sqlInfo2.table
      });
    });
    ...
    accountsData.forEach(account => {
      ...
      tempData.push({
        ...
        serverName: 'P21',
        sqlExpression: sqlInfo3.sql,
        tableName: sqlInfo3.table
      });
    });
    ...
    // Add inventory data
    const inventoryData = getInventoryValue().filter(item => item.category !== '108' && item.category !== '109');
    ...
    const webData = getWebMetrics();
    webData.forEach(web => {
      const sqlInfo1 = getP21SqlInfo('Web Metrics', 'W_Orders', web.month);
      tempData.push({
        chartName: 'Web Metrics',
        variableName: 'W_Orders',
        value: web.W_Orders,
        x_axis: getXAxisValue('Web Metrics', web.month, 'W_Orders'),
        calculation: getCalculation('Web Metrics', 'W_Orders', web.month),
        serverName: 'P21',
        sqlExpression: sqlInfo1.sql,
        tableName: sqlInfo1.table
      });
      const sqlInfo2 = getP21SqlInfo('Web Metrics', 'W_Revenue', web.month);
      tempData.push({
        chartName: 'Web Metrics',
        variableName: 'W_Revenue',
        value: web.W_Revenue,
        x_axis: getXAxisValue('Web Metrics', web.month, 'W_Revenue'),
        calculation: getCalculation('Web Metrics', 'W_Revenue', web.month),
        serverName: 'P21',
        sqlExpression: sqlInfo2.sql,
        tableName: sqlInfo2.table
      });
    ...
    // Add sequential IDs to the filtered data
