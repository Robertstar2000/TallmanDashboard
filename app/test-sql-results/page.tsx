...
export default function TestSQLResultsPage() {
  ...
  return (
    <div className="container mx-auto p-4">
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={4}>
        <Typography variant="h4">SQL Expression Test Results</Typography>
        <Link href="/">
      ...
      {loading ? (
        ...
        <>
          {apiError && (
            <Paper elevation={3} className="p-4 mb-4 bg-red-100">
          ...
          <Paper elevation={3} className="p-4 mb-4">
            ...
            {results.length > 0 ? (
              results.map((result, index) => (
                <Box key={index} className="border p-3 rounded mb-3">
              ...
              <Typography>No successful queries.</Typography>
            )}
          </Paper>
          ...
          {errors.length > 0 && (
            <Paper elevation={3} className="p-4 mb-4">
              ...
              {errors.map((error, index) => (
                <Box key={index} className="border border-red-300 p-3 rounded mb-3">
