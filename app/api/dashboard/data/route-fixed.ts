...
import { SpreadsheetRow, DashboardData, MetricItem } from '@/lib/types/dashboard';
...
export async function GET() {
  try {
    ...
    sortedData.forEach(row => {
      ...
      switch (row.chartName) {
        ...
        case 'Historical Data':
          ...
          let historicalDataPoint = dashboardData.historicalData.find(h => h.month === historicalMonth);
          ...
          if (row.server === 'P21') {
            historicalDataPoint.p21 = value;
          } else if (row.server === 'POR') {
            historicalDataPoint.por = value;
          }
          
          console.log(`  Updated historical data for ${historicalMonth}: P21=${historicalDataPoint.p21}, POR=${historicalDataPoint.por}`);
          break;
        ...
        case 'Accounts':
          ...
          if (row.variableName === 'Current') {
            accountDataPoint.current = value;
          } else if (row.variableName === '30 Days') {
            accountDataPoint.past_due_30 = value;
          } else if (row.variableName === '60 Days') {
            accountDataPoint.past_due_60 = value;
          } else if (row.variableName === '90 Days') {
            accountDataPoint.past_due_90 = value;
          }
          break;
        ...
        case 'Customer Metrics':
          dashboardData.customerMetrics.push({
            ...
            date: new Date().toISOString().split('T')[0],
            name: row.variableName || `Metric ${dashboardData.customerMetrics.length + 1}`,
            newCustomers: value,
            returning: 0
          });
          break;
        ...
        case 'Inventory':
          dashboardData.inventory.push({
            ...
            name: row.variableName || `Category ${dashboardData.inventory.length + 1}`,
            inStock: value,
            onOrder: 0
          });
          break;
        ...
        case 'POR Overview':
          dashboardData.porOverview.push({
            ...
            date: new Date().toISOString().split('T')[0],
            name: row.variableName || `Metric ${dashboardData.porOverview.length + 1}`,
            newRentals: value,
            openRentals: 0,
            rentalValue: 0
          });
        ...
        case 'AR Aging':
          // Find the AR aging data point for the current date or create a new one
          const arDate = new Date().toISOString().split('T')[0];
          let arDataPoint = dashboardData.arAging.find(a => a.name === arDate);
          ...
          // Update the appropriate field based on the variableName
          if (row.variableName === 'Current') {
            arDataPoint.current = value;
            arDataPoint.amount += value;
          } else if (row.variableName === '30 Days') {
            arDataPoint.days_30 = value;
            arDataPoint.amount += value;
          } else if (row.variableName === '60 Days') {
            arDataPoint.days_60 = value;
            arDataPoint.amount += value;
          } else if (row.variableName === '90 Days') {
            arDataPoint.days_90 = value;
            arDataPoint.amount += value;
          } else if (row.variableName === '120 Days') {
            arDataPoint.days_120 = value;
            arDataPoint.amount += value;
          }
          break;
        ...
        case 'Daily Orders':
          ...
          if (row.variableName) {
            ...
            // Check for day names
              ...
              if (lowerVarName.includes('sun')) dayNumber = '1';
              else if (lowerVarName.includes('mon')) dayNumber = '2';
              else if (lowerVarName.includes('tue')) dayNumber = '3';
              else if (lowerVarName.includes('wed')) dayNumber = '4';
              else if (lowerVarName.includes('thu')) dayNumber = '5';
              else if (lowerVarName.includes('fri')) dayNumber = '6';
              else if (lowerVarName.includes('sat')) dayNumber = '7';
            }
          }
    ...
    const totalSiteValue = dashboardData.siteDistribution.reduce((total, site) => total + site.value, 0);
    dashboardData.siteDistribution.forEach(site => {
      site.percentage = totalSiteValue > 0 ? (site.value / totalSiteValue) * 100 : 0;
    });
