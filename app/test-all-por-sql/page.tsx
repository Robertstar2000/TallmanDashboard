...
function fixPORSqlExpression(sql: string, availableTables: string[] = []): string {
  if (!sql) return sql;
...
export default function TestAllPORSQLPage() {
  ...
  const testAllExpressions = async () => {
    ...
    const fixedExpressions = porExpressions.map(expr => ({
      ...expr,
      ...
      <TabPanel value={tabValue} index={0}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {availableTables.map((table, index) => (
            <Chip key={index} label={table} color="primary" variant="outlined" />
          ))}
        </div>
      </TabPanel>
        ...
        <List>
          {testResults.map((result, index) => (
            <Paper key={index} elevation={2} sx={{ mb: 2, p: 2 }}>
              <ListItem 
                sx={{ cursor: 'pointer' }}
                onClick={() => selectExpression(result)}
              >
                <ListItemText
                  primary={`${result.dataPoint} (ID: ${result.id})`}
                  secondary={
                    <>
        ...
        <List>
          {testErrors.map((error, index) => (
            <Paper key={index} elevation={2} sx={{ mb: 2, p: 2 }}>
              <ListItem 
                sx={{ cursor: 'pointer' }}
                onClick={() => selectExpression(error)}
              >
                <ListItemText
                  primary={`${error.dataPoint} (ID: ${error.id})`}
                  secondary={
                    <>
                      <Typography component="span" variant="body2" color="textPrimary">
                        Original SQL:
