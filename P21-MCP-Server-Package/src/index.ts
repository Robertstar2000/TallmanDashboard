  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        ...
        switch (request.params.name) {
          ...
          case 'list_tables': {
            try {
              ...
              let dbName = 'Unknown';
              try {
                const dbRes: any = await connection.query("SELECT DB_NAME() as db");
                dbName = dbRes?.[0]?.db || dbRes?.[0]?.DB || 'Unknown';
              } catch (e) {
                console.error('[P21] Failed to get DB_NAME():', (e as Error)?.message || e);
              }
              ...
              console.error('[P21] list_tables error:', (err as Error)?.message || err);
              throw new McpError(
                ErrorCode.InternalError,
                `Failed to list tables: ${(err as Error)?.message || 'Unknown error'}`
              );
            }
          }
        ...
        throw new McpError(
          ErrorCode.InternalError,
          `P21 SQL execution failed: ${error instanceof Error ? error.message : 'Unknown database error'}. No fallback data available - check database connection and query syntax.`
        );
      }
    });
  }
